...
theorem specializationLe_generateOpen (f g : C(X, Y)) (hfg : ∀ x, SpecializationLe (f x) (g x))
...
theorem pointwise_sup_preservesDirectedSup (f g : ScottMap D D') :
  ...
  intro S hS hSdir
  change f (sSup S) ⊔ g (sSup S) = sSup (Set.image (fun x => f x ⊔ g x) S)
...
theorem pointwise_sSup_preservesDirectedSup (F : Set (ScottMap D D')) :
    PreservesDirectedSup (fun x => sSup (Set.image (fun f : ScottMap D D' =>
      (f : D → D') x) F)) := by
  intro S hS hSdir
...
lattice. -/
noncomputable def theorem33sSup (F : Set (ScottMap D D')) : ScottMap D D' :=
  ScottMap.sSupMaps F

/-- **Scott 1972, Theorem 3.3 (binary join).** Pointwise join of Scott-continuous
maps is
Scott-continuous. -/
noncomputable def theorem33Sup (f g : ScottMap D D') : ScottMap D D' :=
  ScottMap.sup f g
...
`e ≪ x`, else
`⊥`. We encode the conditional value as `⨆ _ : e ≪ x, e'` (which is `e'` when `e ≪
x` and `⊥`
...
theorem stepMap_le_of_wayBelow {e : D} {e' : D'} {g : ScottMap D D'}
  ...
  · change stepFun e e' x ≤ (g : D → D') x
    rw [stepFun_of_not_wayBelow hex]
    exact bot_le

/-- **The step function is way below `f`.** If `e' ≪ f e`, then `ē[e, e'] ≪ f` in
`[D → D']`,
witnessed by the Scott-open set `{g | e' ≪ g e}` (open because suprema in `[D →
D']` are pointwise
...
of a continuous
lattice: `φ ≪ g` forces `φ ≤ ⊔ᵢ ē[eᵢ,eᵢ']` for finitely many pairs with `eᵢ' ≪ g
eᵢ`, and the
...
/-- Subbasic sets of the pointwise (product) topology on `[D → D']`: `{f | f x ∈
U}` for `U`
Scott-open in `D'`. -/
...
theorem theorem_3_3_topology (hD : IsContinuousLattice D) (hD' : IsContinuousLattice D') :
    (scottTopologicalSpace : TopologicalSpace (ScottMap D D')) =
      scottMapPointwiseTopology D D' := by
  ...
  · -- Scott ⊆ pointwise: each Scott-open lattice set is pointwise-open, via the `↟φ` basis
    ...
    · exact @isOpen_biInter_finset (ScottMap D D') (D × D') (scottMapPointwiseTopology D D') F _
        (fun p _ =>
          pointwiseSubbasic_isOpen p.1
            (isOpen_iff_scottOpen.mpr (scottOpen_wayBelow p.2)))
...
theorem scottFunctionSubbasis_isOpen_scott {x : D} {U : Set D'}
    (hU : @IsOpen D' scottTopologicalSpace U) :
...
/-- **Scott 1972, Corollary 3.4 (joint continuity, core).** Evaluation `[D → D'] ×
D → D'`,
...
/-- **Scott 1972, Prop 2.10 / March 1972 correction (p. 135).** For directed `S ⊆
D`,
...
continuous lattice. For `d ∈ D` we have, in the ambient `D'`, `i(d) = ⊔{x' | x' ≪
i(d)}`; applying
the retraction `j` (which preserves this directed supremum) gives `d = ⊔{j(x') |
x' ≪ i(d)}` in
...
* `scott ≤ induced`: each induced-open `i⁻¹V'` is Scott-open in `D` because `i` is
a Scott map.
* `induced ≤ scott`: the sets `i⁻¹(↟x') = {z | x' ≪ i(z)}` (`x' ∈ D'`) are a basis
of `D`'s Scott
  topology — given a Scott-open `U ∋ d`, the directed family `{j(x') | x' ≪ i(d)}`
  (sup `d`) meets
  `U` at some `j(x')`, and `i⁻¹(↟x') ⊆ U` since `x' ≪ i(z) ⟹ j(x') ⊑ z` with `U`
  upper. Each such
  basic set is induced-open by construction, so every Scott-open of `D` is
  induced-open. -/
...
/-- **Scott 1972, Proposition 3.7 (projection half).** Under the same hypotheses
with
...
def functionSpace (R₀ : IsContinuousLatticeRetraction D₀ D₀')
...
def functionSpace (P₀ : IsContinuousLatticeProjection D₀ D₀')
    (P₁ : IsContinuousLatticeProjection D₁ D₁') :
    ScottMapProjection D₀ D₀' D₁ D₁' where
  toScottMapRetraction :=
    ScottMapRetraction.functionSpace P₀.toIsContinuousLatticeRetraction
      P₁.toIsContinuousLatticeRetraction
  incl_retr_le f' y := by
...
/-- **Scott 1972, Proposition 3.7 (retraction).** Function spaces inherit
retractions. -/
def proposition37Retraction (R₀ : IsContinuousLatticeRetraction D₀ D₀')
    (R₁ : IsContinuousLatticeRetraction D₁ D₁') :
    ScottMapRetraction D₀ D₀' D₁ D₁' :=
  ScottMapRetraction.functionSpace R₀ R₁

/-- **Scott 1972, Proposition 3.7 (projection).** Function spaces inherit
projections. -/
def proposition37Projection (P₀ : IsContinuousLatticeProjection D₀ D₀')
    (P₁ : IsContinuousLatticeProjection D₁ D₁') :
...
/-- **Scott 1972, Proposition 3.10(ii).** The inclusion map of a projection is
injective. -/
theorem incl_injective (P : IsContinuousLatticeProjection D D') {x y : D} (h : P.incl x =
  P.incl y) : x = y := by
...
/-- **Scott 1972, Proposition 3.10(iii).** Projections preserve the way-below
relation. -/
...
`i(j x') ⊑ x'`
(by `i ∘ j ⊑ id`), so it is a member of the set; `≥`: each member `x` (with `i x ⊑
x'`) satisfies
...
/-- **Scott 1972, Proposition 3.10(iv).** Scott's candidate retraction `j(x') = ⊔
{ x | i(x) ⊑
x' }`. -/
...
/-- **Scott 1972, Proposition 3.8.** Scott's maximal subspace extension `fbar : Y →
D`. -/
...
theorem scottSubspaceExtendInf_eq_of_ext {e : X → Y}
...
omit [CompleteLattice Y] [TopologicalSpace X] [TopologicalSpace Y] in
/-- **Scott 1972, Lemma 3.9 (inclusion at infimum level).** Used in the
inverse-limit
...
/-- `J_D` is closed under binary joins (Scott 1972, 3.12). The key step: since `j
x ⊔ k x ⊑ x`,
monotonicity and idempotency force `j (j x ⊔ k x) = j x` and `k (j x ⊔ k x) = k
x`. -/
...
noncomputable def projection : IsContinuousLatticeProjection D (ScottMap D D) where
  ...
  incl_retr_le := fun f => by
    rw [ScottMap.le_def]
    intro y
    change ((ScottMap.const ((f : D → D) ⊥) : ScottMap D D) : D → D) y ≤ (f : D → D) y
...
/-- `fix` is monotone in `f`: if `f ⊑ f'` then `fix f ⊑ fix f'`, since `f (fix f')
⊑ f' (fix f') =
...
that `a` is a
pre-fixed point of `g`. Now `g a = ⊔_{f∈S} f a` (pointwise sup), and each `f a =
⊔_{f'∈S} f (fix f')`
(continuity of `f` on the directed family `{fix f'}`); for any `f, f' ∈ S` pick `h
∈ S` above both,
...
/-- **Scott 1972, Proposition 3.14.** The fixed-point operator as a
