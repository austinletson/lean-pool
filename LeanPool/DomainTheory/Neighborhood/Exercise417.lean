...
(For algebraists.) Let `⟨S, 1, ·⟩` be a monoid (a semi-group with unit). Then `P
S` is a domain — in
...
/-- The least solution `lfpSet (F a b)` indeed solves Scott's equation `x = {1} ∪
{a, b} ∪ x ·
x`. -/
...
/-- **Exercise 4.17 (Scott 1981, PRG-19).** The least solution of `x = {1} ∪ {a,
b} ∪ x · x` is the
submonoid generated by `{a, b}` (all finite products of `a`s and `b`s, including
`1`). -/
...
/-- In any monoid, `Set.univ` is a fixed point of `F a b`: `s = 1 · s ∈ univ ·
univ`. -/
