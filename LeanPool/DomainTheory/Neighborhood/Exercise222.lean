...
We follow Scott's hint. Fix `C : Set (Set τ)` (non-empty) closed under non-empty
intersections
(`hInter`) and directed unions (`hDir`).

* `Δ` = the **finite** sets `F` included in some member of `C` (here the subtype
`Tok C`).
* The **closure** of `F` is `Fbar = ⋂ {X ∈ C ∣ F ⊆ X}` (`Cl C F`); since `F` lies in
some member of
  `C`, this intersection is over a non-empty family, so `Fbar ∈ C` (`Cl_mem`). The
  `Fbar` are the "finite"
  elements.
* The neighbourhood system `reprSystem` over `Δ` has neighbourhoods `C(F) = {G ∈ Δ
∣ F ⊆ Gbar}`
  (`nbhd C F`).

The representation isomorphism `reprIso : reprSystem.Element ≃o C` (under `⊆`)
sends an ideal element
`x` to `⋃ {Fbar ∣ C(F) ∈ x}` and a set `X ∈ C` to the element generated by `{C(F) ∣
F ⊆ X}`. The key
identity (Scott) is `X = ⋃ {Fbar ∣ F ⊆ X, F ∈ Δ}`, realized by
...
/-- The **closure** `Fbar = ⋂ {X ∈ C ∣ F ⊆ X}` of a set `F` relative to the family
`C`. -/
...
include hInter in
/-- Every closure `Fbar` of a token lies in `C` (closure under non-empty
intersection). -/
...
include hInter in
/-- `C(F) ⊆ C(G) ↔ G ⊆ Fbar`: comparison of neighbourhoods reduces to inclusion
against closures. -/
...
/-- **The representation neighbourhood system** `reprSystem` over `Δ = Tok C`:
neighbourhoods are
...
/-- The family `{Fbar ∣ C(F) ∈ x}` of finite-element closures present in the ideal
element `x`. -/
def famC (x : (reprSystem C hInter hne).Element) : Set (Set τ) :=
  {Y | ∃ F : Tok C, x.mem (nbhd C F.1) ∧ Y = Cl C F.1}

/-- Scott's `⋃ {Fbar ∣ C(F) ∈ x}`: the set of `C` represented by the ideal element
`x`. -/
...
`C(F₃) ∈ x` whose
closure dominates both `Fbar` and `Fbar'`. (From the filter intersection `C(F) ∩ C(F')
∈ x`, which is
some `C(F₃)`.) -/
...
/-- The empty token's neighbourhood `C(∅) = Δ` is in every element (it is the
master). -/
...
/-- **Finite sets are captured by a single token.** A finite `s ⊆ toC x` is
...
Reverse: `G` is
finite, so the finite-subcover lemma gives `F` with `C(F) ∈ x` and `G ⊆ Fbar`,
whence `C(F) ⊆ C(G)`
...
/-- The ideal element generated by `X ∈ C`: it contains the neighbourhoods above
some `C(F)` with
`F ⊆ X`. -/
...
/-- **Scott's identity** `X = ⋃ {Fbar ∣ F ⊆ X, F ∈ Δ}` in round-trip form: `toC (ofC
X) = X`. -/
...
include hDir in
/-- `toC x ∈ C`: the represented set lies in the family (closure under directed
unions). -/
theorem toC_mem (x : (reprSystem C hInter hne).Element) : toC C hInter hne x ∈ C := by
  change ⋃₀ famC C hInter hne x ∈ C
...
include hDir in
/-- **Exercise 2.22 — the representation theorem.** `C`, ordered by inclusion, is
order-isomorphic
to the domain `|reprSystem|`. The isomorphism sends `x ↦ ⋃ {Fbar ∣ C(F) ∈ x}`
(`toC`) and
`X ↦ {C(F) ∣ F ⊆ X}` (`ofC`). -/
def reprIso : (reprSystem C hInter hne).Element ≃o {X : Set τ // X ∈ C} where
  ...
  map_rel_iff' := by
    intro a b
    change toC C hInter hne a ⊆ toC C hInter hne b ↔ a ≤ b
    constructor
    · intro hsub N hN
...
end Domain.Neighborhood.Exercise222
