...
/-- **Example 2.4 — the run-eliminating mapping `g : B → B`.** `X g Y` iff `X =
σΣ*` and the cone of
the guaranteed output `(out σ)Σ*` approximates `Y`. Definition 2.1: (i) `out [] =
[]` so `Δ g Δ`;
...
end Domain.Neighborhood.Example24
