...
where `valElt none = ⊥`, `valElt (some true) = true`, `valElt (some false) =
false`. The cone `σΣ*`
...
(weakly) more defined
value: `valElt (scan σ) ⊑ valElt (scan σ')`. If `scan σ = none` then `valElt = ⊥ ⊑
_`; otherwise the
...
end Domain.Neighborhood.Example23
