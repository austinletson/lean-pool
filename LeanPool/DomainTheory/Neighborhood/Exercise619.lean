...
> **EXERCISE 6.19.** For the sake of uniformity restrict attention to systems `𝒟`
on sets
> `Δ ⊆ {0,1}*`, where `Λ ∈ Δ` and `∅ ∉ 𝒟`, and to the category of strict maps.
Define sum and
> product by
...
We reuse the single-bit prefix `embBit b X = bX` of Example 6.2 (so `0X = embBit
false X`,
...
`D₀ + D₁ = {{Λ} ∪ 0Δ₀ ∪ 1Δ₁} ∪ {0X ∣ X ∈ 𝒟₀} ∪ {1Y ∣ Y ∈ 𝒟₁}`, with `Λ = []`, `0X
= embBit false X`,
`1Y = embBit true Y`. -/
...
theorem embT_subset_sumTokMaster {Y : Set Str} (hY : D₁.mem Y) :
  ...
  (embBit_subset.mpr (D₁.sub_master hY)).trans
    (Set.subset_union_right.trans (Set.subset_insert _ _))
...
master `{Λ} ∪ 0Δ₀ ∪ 1Δ₁`, a left copy `0X` (`X ∈ 𝒟₀`), or a right copy `1Y` (`Y ∈
𝒟₁`). The standing
...
def toSum (x : (sumTok D₀ D₁ h₀ h₁).Element) : (sum D₀ D₁ h₀ h₁).Element where
  ...
  inter_mem := by
    ...
    · refine Or.inr (Or.inl ⟨X ∩ X', ?_, by rw [inj₀_inter], ?_⟩)
      · have hz := x.inter_mem hzX hzX'
        rw [embBit_inter] at hz
        exact sumTok_mem_embF_inv (x.sub hz)
    ...
    · refine Or.inr (Or.inr ⟨Y ∩ Y', ?_, by rw [inj₁_inter], ?_⟩)
      · have hz := x.inter_mem hzY hzY'
        rw [embBit_inter] at hz
        exact sumTok_mem_embT_inv (x.sub hz)
...
/-- Scott's (1) for the product: inclusion of product neighbourhoods is
componentwise. -/
...
theorem pairTok_fstTok_sndTok (z : (prodTok D₀ D₁).Element) :
  pairTok (fstTok z) (sndTok z) = z := by
  apply NeighborhoodSystem.Element.ext
  intro W
  constructor
