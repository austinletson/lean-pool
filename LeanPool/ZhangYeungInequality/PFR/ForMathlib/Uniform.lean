...
/-- The assertion that the law of $X$ is the uniform probability measure on a finite set
$H$.
While in applications $H$ will be non-empty finite set, $X$ measurable, and and $μ$ a
probability
measure, it could be technically convenient to have a definition that works even without
these
hypotheses. (For instance, `isUniform` would be well-defined, but false, for infinite
`H`).
...
lemma exists_isUniform [MeasurableSpace S] [MeasurableSingletonClass S]
    ...
    ∃ (Ω : Type uS) (_ : MeasurableSpace Ω) (X : Ω → S) (μ : Measure Ω),
      IsProbabilityMeasure μ ∧ Measurable X ∧ IsUniform H X μ ∧ (∀ ω,
        X ω ∈ H) ∧ FiniteRange X := by
...
-/
lemma _root_.ProbabilityTheory.IsUniform.comp
    [DecidableEq T] {H : Finset S} (h : IsUniform H X μ) {f : S → T}
...
lemma exists_isUniform_measureSpace {S : Type uS} [MeasurableSpace S]
    ...
    IsProbabilityMeasure (ℙ : Measure Ω) ∧ Measurable U ∧ IsUniform H U ∧ (∀ ω : Ω,
      U ω ∈ H) ∧
      FiniteRange U := by
  rcases exists_isUniform H h with ⟨Ω, mΩ, X, μ, hμ, Xmeas, Xunif, Xmem, Xfin⟩
  exact ⟨Ω, ⟨μ⟩, X, hμ, Xmeas, Xunif, Xmem, Xfin⟩

/-- Uniform distributions exist, version with a Finite set rather than a Finset and
giving a
measure
space -/
lemma exists_isUniform_measureSpace' {S : Type uS} [MeasurableSpace S]
...
lemma _root_.ProbabilityTheory.IsUniform.ae_mem (h : IsUniform H X μ) : ∀ᵐ ω ∂μ,
  X ω ∈ H := h.measure_preimage_compl

/-- Uniform random variables only exist for non-empty sets H. -/
lemma _root_.ProbabilityTheory.IsUniform.nonempty
    {H : Finset S} (h : IsUniform H X μ) [hμ : NeZero μ] : H.Nonempty := by
...
/-- A "unit test" for the definition of uniform distribution. -/
lemma _root_.ProbabilityTheory.IsUniform.measure_preimage_of_mem
...
/-- A "unit test" for the definition of uniform distribution. -/
lemma _root_.ProbabilityTheory.IsUniform.measureReal_preimage_of_mem
    {A : Finset S} [IsProbabilityMeasure μ]
...
lemma _root_.ProbabilityTheory.IsUniform.measure_preimage
    {H : Finset S} (h : IsUniform H X μ) (hX : Measurable X)
  ...
  _ = (μ univ) * (Nat.card (H' ∩ H).Elem) / Nat.card H := by
    rw [Finset.sum_const, Nat.card_eq_card_finite_toFinset, nsmul_eq_mul, ← mul_assoc,
      mul_one_div]

/-- $\mathbb{P}(U_H \in H') = \dfrac{|H' \cap H|}{|H|}$ -/
lemma _root_.ProbabilityTheory.IsUniform.measureReal_preimage
    {H : Finset S} (h : IsUniform H X μ) (hX : Measurable X)
    (H' : Set S) :
    μ.real (X ⁻¹' H') = (μ.real univ) * (Nat.card (H' ∩ H).Elem) / Nat.card H := by
  simp [measureReal_def, h.measure_preimage hX H', ENNReal.toReal_div]

lemma _root_.ProbabilityTheory.IsUniform.nonempty_preimage_of_mem
    [NeZero μ] {H : Finset S} (h : IsUniform H X μ)
...
lemma _root_.ProbabilityTheory.IsUniform.full_measure (h : IsUniform H X μ) (hX : Measurable X) :
...
lemma _root_.ProbabilityTheory.IsUniform.of_identDistrib
    {Ω' : Type*} [MeasurableSpace Ω'] (h : IsUniform H X μ)
    {X' : Ω'
      → S} {μ' : Measure Ω'} (h' : IdentDistrib X X' μ μ') (hH : MeasurableSet (H : Set S)) :
    IsUniform H X' μ' := by
  constructor
  · intro x hx y hy
...
-/
lemma _root_.ProbabilityTheory.IsUniform.measure_preimage_ne_zero
...
by
$H'$ on $H' \cap H$. -/
lemma _root_.ProbabilityTheory.IsUniform.restrict
    {H : Set S} (h : IsUniform H X μ) (hX : Measurable X) (H' : Set S) :
...
lemma _root_.ProbabilityTheory.IdentDistrib.of_isUniform
...
lemma _root_.ProbabilityTheory.IsUniform.map_eq_uniformOn [Countable S] [IsProbabilityMeasure μ]
...
lemma isUniform_iff_map_eq_uniformOn [Finite H] {Ω : Type*} [mΩ : MeasurableSpace Ω] (μ :
  Measure Ω)
