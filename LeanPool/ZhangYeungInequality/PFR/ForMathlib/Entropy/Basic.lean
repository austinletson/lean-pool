...
* `H[X | Y] = condEntropy X Y`, such that `H[X | Y] = (volume.map Y)[fun y ↦ H[X | Y ←
  y]]`
* `I[X : Y] = mutualInfo X Y`

All notations have variants where we can specify the measure (which is otherwise
supposed to be `volume`). For example `H[X; μ]` and `I[X : Y; μ]` instead of `H[X]`
and
`I[X : Y]` respectively.
...
@[inherit_doc entropy] notation3:max "H[" X " | " Y " ← " y "; " μ "]" =>
  entropy X (μ[|Y ← y])
...
lemma _root_.ProbabilityTheory.IdentDistrib.entropy_congr
    {Ω' : Type*} [MeasurableSpace Ω'] {μ' : Measure Ω'} {X' :
  Ω' → S}
...
/-- Entropy is at most the logarithm of the cardinality of a set in which X almost
surely takes
values in. -/
...
One can also use `entropy_of_comp_eq_of_comp` as an alternative if verifying injectivity
is fiddly.
...
lemma _root_.ProbabilityTheory.IsUniform.entropy_eq
    [DiscreteMeasurableSpace S] {H : Finset S} {X : Ω
  → S} {μ : Measure Ω}
  ...
  have (t : S) : negMulLog ((μ.map X).real {t}) = (μ.map X).real {t} * log (Nat.card H) := by
    ...
    · simp [negMulLog, map_measureReal_apply hX' (.singleton t),
      hX.measureReal_preimage_of_nmem ht]
  rw [entropy_eq_sum_finset' (A := H), Finset.sum_congr rfl (fun t _ ↦ this t),
    ← Finset.sum_mul,
...
-/
lemma _root_.ProbabilityTheory.IsUniform.entropy_eq' [DiscreteMeasurableSpace S]
...
/-- If `X` is `S`-valued random variable, then `H[X] = log |S|` if and only if `X` is
uniformly
distributed. -/
...
lemma prob_ge_exp_neg_entropy [MeasurableSingletonClass S] (X : Ω → S) (μ : Measure Ω)
  ...
  have h_invnorm_finite : norm⁻¹ ≠ ∞ := by
    ...
    exact h_norm_pos.ne'
  have h_pdf_finite : ∀ s,
    pdf_nn s ≠ ∞ := fun s ↦ ENNReal.mul_ne_top h_invnorm_finite (h_finite s)
  have h_norm_cancel : norm * norm⁻¹ = 1 :=
    ENNReal.mul_inv_cancel h_norm_pos.ne' h_norm_finite.ne
...
/-- If `X` is an `S`-valued random variable of non-positive entropy, then `X` is almost
surely
constant. -/
lemma const_of_nonpos_entropy [MeasurableSingletonClass S]
    {Ω : Type*} [MeasurableSpace Ω] {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω
      → S} (hX : Measurable X) [FiniteRange X] (hent : H[X; μ] ≤ 0) :
    ∃ s : S, μ.real (X ⁻¹' {s}) = 1 := by
...
This is the expectation under the law of `Y` of the entropy of the law of `X`
conditioned on the
event `Y = y`. -/
noncomputable
def _root_.ProbabilityTheory.condEntropy
...
lemma _root_.ProbabilityTheory.condEntropy_eq_zero
    (hY : Measurable Y) (μ : Measure Ω) [IsFiniteMeasure μ] (t : T)
...
/--
Conditional entropy of a random variable is equal to the entropy of its conditional
kernel.
-/
lemma _root_.ProbabilityTheory.condEntropy_eq_kernel_entropy
...
lemma _root_.ProbabilityTheory.condEntropy_two_eq_kernel_entropy
...
@[simp]
lemma _root_.ProbabilityTheory.condEntropy_zero_measure
    (X : Ω → S) (Y : Ω → T) : H[X | Y; (0 : Measure Ω)] = 0 :=
  by simp [condEntropy]

/-- Conditional entropy is non-negative. -/
lemma _root_.ProbabilityTheory.condEntropy_nonneg
    (X : Ω → S) (Y : Ω → T) (μ : Measure Ω) : 0 ≤ H[X | Y; μ] :=
...
lemma _root_.ProbabilityTheory.condEntropy_le_log_card [MeasurableSingletonClass S] [Fintype S]
...
lemma _root_.ProbabilityTheory.condEntropy_eq_sum
    [MeasurableSingletonClass T] (X : Ω → S) (Y : Ω
  → T) (μ : Measure Ω)
    [IsFiniteMeasure μ] (hY : Measurable Y) [FiniteRange Y] :
    H[X | Y; μ] = ∑ y ∈ FiniteRange.toFinset Y,
      ((μ.map Y).real {y}) * H[X | Y ← y; μ] := by
  rw [condEntropy_def, integral_eq_setIntegral (ae_mem_of_finiteRange hY),
    setIntegral_finset _ .finset]
  simp_rw [smul_eq_mul]

/-- `H[X|Y] = ∑_y P[Y=y] H[X|Y=y]`. -/
lemma _root_.ProbabilityTheory.condEntropy_eq_sum_fintype
...
lemma _root_.ProbabilityTheory.condEntropy_prod_eq_sum
    {X : Ω → S} {Y : Ω → T} {Z : Ω → T'} [MeasurableSpace T']
    [MeasurableSingletonClass T']
...
lemma _root_.ProbabilityTheory.condEntropy_eq_sum_sum
    (hX : Measurable X) {Y : Ω → T} (hY : Measurable Y)
...
/-- `H[X|Y] = ∑_y ∑_x P[Y=y] P[X=x|Y=y] log ⧸(P[X=x|Y=y])`$. -/
lemma _root_.ProbabilityTheory.condEntropy_eq_sum_sum_fintype {Y : Ω → T} (hY : Measurable Y)
...
-/
lemma _root_.ProbabilityTheory.condEntropy_eq_sum_prod (hX : Measurable X) {Y : Ω → T}
    (hY : Measurable Y)
    (μ : Measure Ω) [IsProbabilityMeasure μ] [FiniteRange X] [FiniteRange Y] :
...
lemma _root_.ProbabilityTheory.condEntropy_of_injective
...
lemma _root_.ProbabilityTheory.condEntropy_comp_of_injective {Y : Ω → U}
    (μ : Measure Ω) (hX : Measurable X) (f : S → T) (hf : Injective f) :
    H[f ∘ X | Y; μ] = H[X | Y; μ] :=
  integral_congr_ae (ae_of_all _ (fun _ ↦ entropy_comp_of_injective _ hX f hf))

/-- `H[X, Y| Z] = H[Y, X| Z]`. -/
lemma _root_.ProbabilityTheory.condEntropy_comm [Countable T] {Z : Ω → U}
...
lemma _root_.ProbabilityTheory.chain_rule' (μ : Measure Ω) [IsZeroOrProbabilityMeasure μ]
  ...
  · simp_rw [← Kernel.map_const _ (hX.prodMk hY), Kernel.fst_map_prod _ hY,
...
conditional entropy. -/
lemma _root_.ProbabilityTheory.IdentDistrib.condEntropy_eq
    {Ω' : Type*} [MeasurableSpace Ω'] {X : Ω → S} {Y : Ω → T}
    {μ' : Measure Ω'} {X' : Ω' → S} {Y' : Ω'
      → T} [IsProbabilityMeasure μ] [IsProbabilityMeasure μ']
    (hX : Measurable X) (hY : Measurable Y) (hX' : Measurable X') (hY' : Measurable Y')
    (h : IdentDistrib (⟨X, Y⟩) (⟨X',
      Y'⟩) μ μ') [FiniteRange X] [FiniteRange Y] [FiniteRange X']
    [FiniteRange Y'] :
    H[X | Y; μ] = H[X' | Y'; μ'] := by
...
lemma _root_.ProbabilityTheory.condEntropy_of_injective' (μ : Measure Ω)
...
/-- `H[X | f(X)] = H[X] - H[f(X)]`. -/
lemma _root_.ProbabilityTheory.condEntropy_comp_self
    [IsProbabilityMeasure μ] (hX : Measurable X) {f : S → U}
...
lemma _root_.ProbabilityTheory.cond_chain_rule' (μ : Measure Ω) [IsZeroOrProbabilityMeasure μ]
    (hX : Measurable X) (hY : Measurable Y) (hZ : Measurable Z)
...
To upgrade this to equality, see `entropy_of_comp_eq_of_comp` or
`entropy_comp_of_injective`. -/
lemma _root_.ProbabilityTheory.entropy_comp_le (μ : Measure Ω) [IsZeroOrProbabilityMeasure μ]
...
/-- A Schroder-Bernstein type theorem for entropy : if two random variables are
functions of each
...
lemma _root_.ProbabilityTheory.entropy_of_comp_eq_of_comp
    (μ : Measure Ω) [IsZeroOrProbabilityMeasure μ] (hX : Measurable X) (hY : Measurable Y)
    (f : S → T) (g : T → S) (h1 : Y = f ∘ X) (h2 : X =
      g ∘ Y) [FiniteRange X] [FiniteRange Y] :
    H[X; μ] = H[Y; μ] := by
...
noncomputable
def _root_.ProbabilityTheory.mutualInfo
    (X : Ω → S) (Y : Ω → T) (μ : Measure Ω := by volume_tac) : ℝ :=
  H[X; μ] + H[Y; μ] - H[⟨X, Y⟩; μ]
...
/-- Substituting variables for ones with the same distributions doesn't change the
mutual
information. -/
lemma _root_.ProbabilityTheory.IdentDistrib.mutualInfo_eq
    {Ω' : Type*} [MeasurableSpace Ω'] {μ' : Measure Ω'}
...
/-- The conditional mutual information `I[X : Y| Z]` is the mutual information of `X|
Z=z` and
`Y| Z=z`, integrated over `z`. -/
noncomputable
def _root_.ProbabilityTheory.condMutualInfo
    (X : Ω → S) (Y : Ω → T) (Z : Ω → U) (μ : Measure Ω := by volume_tac) :
    ℝ := (μ.map Z)[fun z ↦ H[X | Z ← z; μ] + H[Y | Z ← z; μ] - H[⟨X,
      Y⟩ | Z ← z; μ]]
...
lemma _root_.ProbabilityTheory.mutualInfo_nonneg
    (hX : Measurable X) (hY : Measurable Y) (μ : Measure Ω)
...
lemma _root_.ProbabilityTheory.entropy_pair_le_add
    (hX : Measurable X) (hY : Measurable Y) (μ : Measure Ω) [FiniteRange X]
    [FiniteRange Y] : H[⟨X, Y⟩; μ] ≤ H[X; μ] + H[Y; μ] :=
  sub_nonneg.1 <| mutualInfo_nonneg hX hY _

/-- `I[X : Y] = 0` iff `X, Y` are independent. -/
lemma _root_.ProbabilityTheory.mutualInfo_eq_zero
    (hX : Measurable X) (hY : Measurable Y) {μ : Measure Ω}
...
lemma _root_.ProbabilityTheory.mutualInfo_const (hX : Measurable X) (c : T)
...
lemma _root_.ProbabilityTheory.entropy_pair_eq_add
    (hX : Measurable X) (hY : Measurable Y) {μ : Measure Ω}
    [IsZeroOrProbabilityMeasure μ] [FiniteRange X] [FiniteRange Y] :
...
lemma _root_.ProbabilityTheory.iIndepFun.entropy_eq_add {Ω S : Type*} [hΩ:
  MeasureSpace Ω] [IsProbabilityMeasure hΩ.volume]
  ...
  calc
    ...
    _ = H[fun ω (i:Fin m) ↦ X i.castSucc ω] + H[X (.last m)] := by
      ...
      exact iIndepFun.finsets_comp' (by simp [T', T]) h_indep hX (show Measurable φ by fun_prop)
...
/-- `I[X : Y] = I[Y : X]`. -/
lemma _root_.ProbabilityTheory.mutualInfo_comm
    (hX : Measurable X) (hY : Measurable Y) (μ : Measure Ω) :
    I[X : Y; μ] = I[Y : X; μ] := by simp_rw [mutualInfo, add_comm, entropy_comm hX hY]

/-- `I[X : Y] = H[X] - H[X|Y]`. -/
lemma _root_.ProbabilityTheory.mutualInfo_eq_entropy_sub_condEntropy
    (hX : Measurable X) (hY : Measurable Y) (μ : Measure Ω)
    [IsZeroOrProbabilityMeasure μ] [FiniteRange X] [FiniteRange Y] :
...
/-- `I[X : Y] = H[Y] - H[Y | X]`. -/
lemma _root_.ProbabilityTheory.mutualInfo_eq_entropy_sub_condEntropy'
    (hX : Measurable X) (hY : Measurable Y) (μ :
  Measure Ω)
    [IsZeroOrProbabilityMeasure μ] [FiniteRange X] [FiniteRange Y] :
...
/-- `H[X] - I[X : Y] = H[X | Y]`. -/
lemma _root_.ProbabilityTheory.entropy_sub_mutualInfo_eq_condEntropy
    (hX : Measurable X) (hY : Measurable Y) (μ :
  Measure Ω)
    [IsZeroOrProbabilityMeasure μ] [FiniteRange X] [FiniteRange Y] :
...
lemma _root_.ProbabilityTheory.IndepFun.condEntropy_eq_entropy
    {μ : Measure Ω} (h : IndepFun X Y μ)
...
/-- The conditional mutual information agrees with the information of the conditional
kernel.
-/
lemma _root_.ProbabilityTheory.condMutualInfo_eq_kernel_mutualInfo
    (hX : Measurable X) (hY : Measurable Y) (hZ : Measurable Z)
...
lemma _root_.ProbabilityTheory.condMutualInfo_eq_sum
    [MeasurableSingletonClass U] [IsFiniteMeasure μ]
...
lemma _root_.ProbabilityTheory.condMutualInfo_nonneg
...
/-- `I[X : Y | Z] = I[Y : X | Z]`. -/
lemma _root_.ProbabilityTheory.condMutualInfo_comm
    (hX : Measurable X) (hY : Measurable Y) (Z : Ω → U) (μ : Measure Ω) :
...
lemma _root_.ProbabilityTheory.condMutualInfo_eq [Countable U]
    (hX : Measurable X) (hY : Measurable Y) (hZ : Measurable Z)
    (μ : Measure Ω) [IsZeroOrProbabilityMeasure μ] [FiniteRange Z] :
...
-/
lemma _root_.ProbabilityTheory.condMutualInfo_of_inj_map
    [Countable U] [IsZeroOrProbabilityMeasure μ]
    (hX : Measurable X) (hY : Measurable Y) (hZ : Measurable Z)
  ...
  rw [← condEntropy_of_injective μ (hX.prodMk hY) hZ g hg,
    ← condEntropy_of_injective μ hX hZ _ hf]

lemma _root_.ProbabilityTheory.condMutualInfo_of_inj [Countable U]
    (hX : Measurable X) (hY : Measurable Y) (hZ : Measurable Z)
    (μ : Measure Ω) [IsZeroOrProbabilityMeasure μ]
    [FiniteRange X] [FiniteRange Y] [FiniteRange Z]
    {V : Type*} [MeasurableSpace V] [MeasurableSingletonClass V] [Countable V]
...
lemma _root_.ProbabilityTheory.condMutualInfo_of_inj'
    {S T U S' T' U' Ω : Type*} [mΩ : MeasurableSpace Ω]
...
lemma _root_.ProbabilityTheory.condEntropy_prod_eq_of_indepFun
    [Finite T] [Finite U] [IsZeroOrProbabilityMeasure μ]
  ...
  have : H[X | Y; μ] = ∑ z, (μ.real (Z ⁻¹' {z})) * H[X | Y; μ] := by
    rw [← Finset.sum_mul,
      sum_measureReal_preimage_singleton _ fun z _ ↦ hZ <| .singleton z]; simp
  rw [this]
  congr with w
...
lemma _root_.ProbabilityTheory.condMutualInfo_eq_zero (hX : Measurable X) (hY : Measurable Y)
...
/-- If `X, Y` are conditionally independent over `Z`, then `H[X, Y, Z] = H[X, Z] + H[Y,
Z] - H[Z]`.
-/
lemma _root_.ProbabilityTheory.ent_of_cond_indep
    (hX : Measurable X) (hY : Measurable Y) (hZ : Measurable Z)
...
/-- `H[X] - H[X|Y] = I[X : Y]` -/
lemma _root_.ProbabilityTheory.entropy_sub_condEntropy
...
lemma _root_.ProbabilityTheory.condEntropy_comp_ge
    [FiniteRange X] [FiniteRange Y] (μ : Measure Ω) [IsZeroOrProbabilityMeasure μ]
    (hX : Measurable X) (hY : Measurable Y) (f : S
      → U) : H[Y | f ∘ X; μ] ≥ H[Y | X; μ] := by
...
lemma _root_.ProbabilityTheory.entropy_triple_add_entropy_le
    (hX : Measurable X) (hY : Measurable Y) (hZ : Measurable Z)
    [FiniteRange X] [FiniteRange Y] [FiniteRange Z] :
...
/-- Let `X, Y` be random variables. For any functions `f, g` on the ranges of `X, Y`
respectively,
we have `I[f ∘ X : g ∘ Y; μ] ≤ I[X : Y; μ]`. -/
lemma mutual_comp_comp_le [Countable U] (μ : Measure Ω) [IsProbabilityMeasure μ] (hX :
  Measurable X)
