...
lemma entropy_map_of_injective [MeasurableSingletonClass U]
    (κ : Kernel T S) (μ : Measure T) {f : S
      → U} (hf : Function.Injective f) (hmes : Measurable f) :
...
lemma entropy_compProd_aux [MeasurableSingletonClass S] [MeasurableSingletonClass T]
    [MeasurableSingletonClass U] {μ} [IsFiniteMeasure μ] {κ :
      Kernel T S} [IsZeroOrMarkovKernel κ]
...
lemma entropy_compProd_deterministic [Countable S] [MeasurableSingletonClass S]
    ...
    (κ : Kernel T S) [IsZeroOrMarkovKernel κ] (μ : Measure T) [IsFiniteMeasure μ] (f :
      T × S → U)
