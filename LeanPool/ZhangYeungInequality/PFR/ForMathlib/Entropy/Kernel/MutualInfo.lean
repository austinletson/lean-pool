...
lemma mutualInfo_compProd
    ...
    [FiniteSupport μ] (hκ : AEFiniteKernelSupport κ μ) (hη :
      AEFiniteKernelSupport η (μ ⊗ₘ κ)) :
...
lemma entropy_snd_sub_mutualInfo_le_entropy_map_of_injective {V : Type*} [Countable V]
    ...
    (κ : Kernel T (S × U)) [IsZeroOrMarkovKernel κ] (μ :
      Measure T) [IsZeroOrProbabilityMeasure μ]
...
lemma _root_.ProbabilityTheory.Kernel.entropy_reverse
    {κ : Kernel T (S × U × V)} [IsZeroOrMarkovKernel κ]
    {μ : Measure T} [IsZeroOrProbabilityMeasure μ] [FiniteSupport μ]
    (hκ : AEFiniteKernelSupport κ μ) :
...
lemma _root_.ProbabilityTheory.Kernel.entropy_condKernel_compProd_triple
    ...
    (μ : Measure T) :
    Hk[condKernel (ξ ⊗ₖ κ ⊗ₖ η) , μ ⊗ₘ (ξ ⊗ₖ κ)] = Hk[η,
      μ ⊗ₘ (ξ ⊗ₖ κ)] :=
  entropy_congr (condKernel_compProd_ae_eq (ξ ⊗ₖ κ) η μ)

-- from kernel (T × S × U) V; Measure (T × S × U)
-- to kernel (T × S) V; Measure (T × S)
lemma _root_.ProbabilityTheory.Kernel.entropy_submodular_compProd
    {ξ : Kernel T S} [IsZeroOrMarkovKernel ξ]
    {κ : Kernel (T × S) U} [IsZeroOrMarkovKernel κ] {η :
      Kernel (T × S × U) V} [IsMarkovKernel η]
    {μ : Measure T} [IsZeroOrProbabilityMeasure μ] [FiniteSupport μ]
    (hκ : AEFiniteKernelSupport κ (μ ⊗ₘ ξ))
    (hη : AEFiniteKernelSupport η (μ ⊗ₘ (ξ ⊗ₖ κ))) (hξ :
      AEFiniteKernelSupport ξ μ) :
    Hk[η, μ ⊗ₘ (ξ ⊗ₖ κ)]
      ≤ Hk[snd (κ ⊗ₖ (comap η MeasurableEquiv.prodAssoc
...
lemma _root_.ProbabilityTheory.Kernel.entropy_compProd_triple_add_entropy_le
    {ξ : Kernel T S} [IsZeroOrMarkovKernel ξ]
    ...
    Hk[(ξ ⊗ₖ κ) ⊗ₖ η, μ] + Hk[ξ, μ]
      ≤ Hk[ξ ⊗ₖ snd (κ ⊗ₖ comap η MeasurableEquiv.prodAssoc
        MeasurableEquiv.prodAssoc.measurable),
          μ] + Hk[ξ ⊗ₖ κ, μ] := by
  rcases eq_zero_or_isProbabilityMeasure μ with rfl | hμ
  · simp
  ...
  · simp only [fst_compProd, entropy_condKernel_compProd_triple]
    ...
    _ ≤ Hk[ξ , μ] + Hk[ξ ⊗ₖ κ , μ]
      + Hk[condKernel (ξ ⊗ₖ snd (κ ⊗ₖ comap η MeasurableEquiv.prodAssoc _)) ,
        μ ⊗ₘ ξ] := by
    ...
    _ = Hk[ξ ,
      μ] + Hk[condKernel (ξ ⊗ₖ snd (κ ⊗ₖ comap η MeasurableEquiv.prodAssoc _)),
      μ ⊗ₘ ξ] +
          Hk[ξ ⊗ₖ κ , μ] := by abel
...
lemma _root_.ProbabilityTheory.Kernel.entropy_triple_add_entropy_le'
    {κ : Kernel T (S × U × V)} [IsZeroOrMarkovKernel κ]
    {μ : Measure T} [IsZeroOrProbabilityMeasure μ] [FiniteSupport μ]
    (hκ : AEFiniteKernelSupport κ μ) :
  ...
  have hξ_eq : ξ = fst κ := by
    rw [ξ_def, hκ'_def, fst_eq, MeasurableEquiv.prodAssoc, MeasurableEquiv.coe_mk,
      Equiv.coe_fn_mk,
  ...
  have h_compProd_triple_eq' :
      ξ ⊗ₖ (κ'' ⊗ₖ comap η MeasurableEquiv.prodAssoc
        MeasurableEquiv.prodAssoc.measurable)
        = κ := by
    rw [← compProd_assoc', h_compProd_triple_eq,hκ'_def, map_map _ (by fun_prop) (by fun_prop)]
    simp
  have h := entropy_compProd_triple_add_entropy_le (ξ := ξ) (κ := κ'') (η := η) (μ :=
    μ) ?_ ?_ ?_
  rotate_left
