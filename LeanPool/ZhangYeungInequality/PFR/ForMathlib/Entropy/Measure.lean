...
* `measureMutualInfo`: mutual information of a measure over a product space, denoted by
  `Im[μ]`,
...
We normalize the measure by `(μ Set.univ)⁻¹` to extend the entropy definition to finite
measures.
What we really want to do is deal with `μ=0` or `IsProbabilityMeasure μ`, but we don't
have
a typeclass for that (we could create one though).
The added complexity ∈ the expression is not an issue because if `μ` is a probability
measure,
...
/--
A measure has finite support if there exists a finite set whose complement has zero
measure.
-/
...
noncomputable
def _root_.MeasureTheory.Measure.finiteSupport
    (μ : Measure S) [hμ : FiniteSupport μ] : Finset S :=
  hμ.finite.choose.filter (μ {·} ≠ 0)

lemma measure_compl_support (μ : Measure S) [hμ : FiniteSupport μ] :
    μ μ.finiteSupportᶜ = 0 := by
  let A := hμ.finite.choose
  have :
      (μ.finiteSupport : Set S)ᶜ
        ⊆ (A : Set S)ᶜ ∪ ⋃ x ∈ A.filter (μ {·} = 0), {x} := by
    intro z hz
    simp only [Measure.finiteSupport, ne_eq, Finset.coe_filter, mem_compl_iff,
      mem_setOf_eq, not_and, Decidable.not_not] at hz
    by_cases h'z : z ∈ A
    · simp [hz h'z, h'z]
    · simp [h'z]
  apply le_antisymm ?_ bot_le
  calc μ (μ.finiteSupport : Set S)ᶜ ≤ μ ((A : Set S)ᶜ ∪ ⋃ x ∈ A.filter (μ {·} = 0),
    {x}) :=
    measure_mono this
...
@[simp] lemma mem_support {μ : Measure S} [hμ : FiniteSupport μ] {x : S} :
    x ∈ μ.finiteSupport ↔ μ {x} ≠ 0 := by
  refine ⟨fun h ↦ ?_, fun h ↦ ?_⟩
  · simp only [Measure.finiteSupport, ne_eq, Finset.mem_filter] at h
    exact h.2
  · contrapose! h
...
instance finiteSupport_of_mul {μ : Measure S} [FiniteSupport μ] (c : ℝ≥0∞) :
    FiniteSupport (c • μ) := ⟨μ.finiteSupport, Measure.ae_smul_measure (ae_mem_support _) _⟩
...
lemma finiteSupport_of_comp
  ...
  classical
  use Finset.image X μ.finiteSupport
...
instance finiteSupport_of_prod {μ : Measure S} [FiniteSupport μ] {ν : Measure T} [SigmaFinite ν]
  ...
  use μ.finiteSupport ×ˢ ν.finiteSupport
  exact Measure.prod_of_full_measure_finset (measure_compl_support μ) (measure_compl_support ν)

/-- The countability hypothesis can probably be dropped here. Proof is unwieldy and can
probably
be golfed. -/
...
theorem _root_.ProbabilityTheory.Measure.ext_iff_singleton_finiteSupport
  ...
  · let A1 := μ1.finiteSupport
    ...
    rw [← hs.coe_toFinset, ← sum_measure_singleton (μ := μ1),
      ← sum_measure_singleton (μ := μ2)]
    simp_rw [h]

theorem _root_.ProbabilityTheory.Measure.ext_iff_measureReal_singleton_finiteSupport
    {μ1 μ2 : Measure S}
...
lemma _root_.ProbabilityTheory.measureEntropy_eq_sum
    {μ : Measure S} {A : Finset S} (hA : μ Aᶜ = 0) :
...
@[simp]
lemma _root_.ProbabilityTheory.measureEntropy_zero
    : Hm[(0 : Measure S)] = 0 := by simp [measureEntropy]

@[simp]
lemma _root_.ProbabilityTheory.measureEntropy_dirac
    [MeasurableSingletonClass S] (x : S) : Hm[Measure.dirac x] = 0 := by
  rw [measureEntropy]
...
lemma _root_.ProbabilityTheory.measureEntropy_of_not_isFiniteMeasure
    (h : ¬ IsFiniteMeasure μ) : Hm[μ] = 0 := by
  simp [measureEntropy, not_isFiniteMeasure_iff.mp h]

lemma _root_.ProbabilityTheory.measureEntropy_of_isProbabilityMeasure
    (μ : Measure S) [IsZeroOrProbabilityMeasure μ] :
...
lemma _root_.ProbabilityTheory.measureEntropy_of_isProbabilityMeasure_finite
    {μ : Measure S} {A : Finset S} (hA :
  μ Aᶜ = 0)
...
lemma _root_.ProbabilityTheory.measureEntropy_univ_smul
    : Hm[(μ Set.univ)⁻¹ • μ] = Hm[μ] := by
...
lemma _root_.ProbabilityTheory.measureEntropy_nonneg (μ : Measure S) : 0 ≤ Hm[μ] := by
  by_cases hμ_fin : IsFiniteMeasure μ
...
/-- Auxiliary lemma for `measureEntropy_le_log_card_of_mem`, which removes the
probability
measure assumption. -/
lemma _root_.ProbabilityTheory.measureEntropy_le_card_aux {μ : Measure S} [IsProbabilityMeasure μ]
...
lemma _root_.ProbabilityTheory.measureEntropy_eq_card_iff_measureReal_eq_aux [Fintype S]
...
lemma _root_.ProbabilityTheory.measureEntropy_eq_card_iff_measure_eq_aux
...
lemma _root_.ProbabilityTheory.measureEntropy_le_log_card_of_mem
    {A : Finset S} (μ : Measure S) (hμA : μ Aᶜ = 0) :
    Hm[μ] ≤ log A.card := by
  have h_log_card_nonneg : 0 ≤ log (Nat.card A) := log_natCast_nonneg (Nat.card ↑A)
  rcases eq_zero_or_neZero μ with rfl|hμ
  · simpa only [measureEntropy_zero] using (log_natCast_nonneg A.card)
  by_cases hμ_fin : IsFiniteMeasure μ
...
lemma _root_.ProbabilityTheory.measureEntropy_le_log_card
    [Fintype S] (μ : Measure S) : Hm[μ] ≤ log (Fintype.card S) :=
  measureEntropy_le_log_card_of_mem (A := (Finset.univ : Finset S)) μ (by simp)

lemma _root_.ProbabilityTheory.measureEntropy_eq_card_iff_measureReal_eq
    [Fintype S] [IsFiniteMeasure μ] [NeZero μ] :
    Hm[μ] = log (Fintype.card S) ↔ ∀ s : S,
      μ.real {s} = μ.real Set.univ / Fintype.card S := by
  rw [← measureEntropy_univ_smul]
...
lemma _root_.ProbabilityTheory.entropy_of_uniformOn (H : Set S) [Nonempty H] [Finite H] :
...
lemma _root_.ProbabilityTheory.measureEntropy_eq_card_iff_measure_eq
    [Fintype S] [IsFiniteMeasure μ] [NeZero μ] :
...
lemma _root_.ProbabilityTheory.measureEntropy_map_of_injective
...
lemma _root_.ProbabilityTheory.measureEntropy_comap
  ...
  classical
  rw [← tsum_range (f :=
    fun x ↦ negMulLog ((μ (Set.range f))⁻¹.toReal * μ.real {x})) (g := f)
...
lemma _root_.ProbabilityTheory.measureEntropy_comap_equiv
    (μ : Measure T) (f : S ≃ᵐ T) : Hm[μ.comap f] = Hm[μ] := by
...
@[simp]
lemma _root_.ProbabilityTheory.measureEntropy_prod
    {μ : Measure S} {ν : Measure T} [FiniteSupport μ] [FiniteSupport ν]
...
noncomputable
def _root_.ProbabilityTheory.measureMutualInfo (μ : Measure (S × T) := by volume_tac) : ℝ :=
...
lemma _root_.ProbabilityTheory.measureMutualInfo_of_not_isFiniteMeasure
    {μ : Measure (S × U)} (h : ¬ IsFiniteMeasure μ) :
    Im[μ] = 0 := by
...
lemma _root_.ProbabilityTheory.measureMutualInfo_univ_smul
    (μ : Measure (S × U)) : Im[(μ Set.univ)⁻¹ • μ] =
  Im[μ] := by
  by_cases hμ_fin : IsFiniteMeasure μ
  swap
...
lemma _root_.ProbabilityTheory.measureMutualInfo_swap (μ : Measure (S × T)) :
  ...
  have : Set.range (Prod.swap : S × T → T × S) = .univ :=
    Set.range_eq_univ.mpr Prod.swap_surjective
  rw [← tsum_univ, ← this,
...
@[simp]
lemma _root_.ProbabilityTheory.measureMutualInfo_prod
    {μ : Measure S} {ν : Measure T} [FiniteSupport μ] [FiniteSupport ν]
...
/-- An ambitious goal would be to replace FiniteSupport with finite entropy. Proof is
long and
slow;
needs to be optimized -/
lemma _root_.ProbabilityTheory.measureMutualInfo_nonneg_aux
    {μ : Measure (S × U)} [FiniteSupport μ]
    [IsZeroOrProbabilityMeasure μ] :
    0 ≤ Im[μ] ∧
    (Im[μ] = 0 ↔ ∀ p,
  ...
  let f (p : S × U) :=
    ((μ.map Prod.fst).real {p.1} * (μ.map Prod.snd).real {p.2})⁻¹ * μ.real {p}
...
lemma _root_.ProbabilityTheory.measureMutualInfo_eq_zero_iff
    {μ : Measure (S × U)} [FiniteSupport μ]
    [IsZeroOrProbabilityMeasure μ] :
    Im[μ] = 0 ↔ ∀ p,
      μ.real {p} = (μ.map Prod.fst).real {p.1} * (μ.map Prod.snd).real {p.2} :=
