...
variable {Ω α β : Type*} {_ : MeasurableSpace Ω} {_ : MeasurableSpace α} {_ :
  MeasurableSpace β}
  {μ : Measure Ω} {A : Ω → α} {B : Ω → β}

/-- If `A` is independent from `B`, then conditioning on an event given by `B` does not
change
the distribution of `A`. -/
theorem _root_.ProbabilityTheory.IndepFun.identDistrib_cond [IsProbabilityMeasure μ]
...
/-- If `A` is independent of `B`, then they remain independent when conditioning on an
event
of the form `A ∈ s` of positive probability. -/
lemma _root_.ProbabilityTheory.IndepFun.cond_left (hi : IndepFun A B μ) {s : Set α}
...
lemma _root_.ProbabilityTheory.IndepFun.cond_right (h : IndepFun A B μ) {t : Set β}
...
lemma _root_.ProbabilityTheory.IndepFun.cond (hi : IndepFun A B μ) {s : Set α} {t : Set β}
    (hs : MeasurableSet s) (ht : MeasurableSet t) (hA : Measurable A) (hB : Measurable B) :
    IndepFun A B (μ[| A ⁻¹' s ∩ B ⁻¹' t]) := by
  apply indepFun_iff_measure_inter_preimage_eq_mul.2 (fun u v hu hv ↦ ?_)
  have I1 : A ⁻¹' s ∩ B ⁻¹' t ∩ (A ⁻¹' u ∩ B ⁻¹' v) =
    A ⁻¹' (s ∩ u) ∩ B ⁻¹' (t ∩ v) := by aesop
...
variable {Ω Ω' α β γ : Type*} [MeasurableSpace Ω] [MeasurableSpace Ω'] [MeasurableSpace α]
  [MeasurableSpace β] [MeasurableSpace γ] {μ : Measure Ω} {f : Ω → α} {g : Ω
    → β} {h : Ω → γ}

/-- The assertion that `f` and `g` are conditionally independent relative to `h`. -/
def _root_.ProbabilityTheory.CondIndepFun
    (f : Ω → α) (g : Ω → β) (h : Ω → γ) (μ : Measure Ω :=
  by volume_tac) : Prop :=
  ∀ᵐ z ∂ (μ.map h), IndepFun f g (μ[|h ← z])

lemma _root_.ProbabilityTheory.condIndepFun_iff : CondIndepFun f g h μ ↔ ∀ᵐ z ∂ (μ.map h),
  IndepFun f g (μ[|h ← z]) := by rfl
...
independent
functions. -/
lemma _root_.ProbabilityTheory.CondIndepFun.comp_right {i : Ω'
  → Ω} (hi : MeasurableEmbedding i) (hi' : ∀ᵐ a ∂μ, a ∈ range i)
...
universe u
variable {Ω : Type*} {α β :
  Type u} [MeasurableSpace Ω] [MeasurableSpace α] [MeasurableSpace β]
...
/--
For `X, Y` random variables, there exist conditionally independent trials `X_1, X_2,
Y'`.
-/
lemma _root_.ProbabilityTheory.condIndep_copies
    (X : Ω → α) (Y : Ω → β) (hX : Measurable X) (hY : Measurable Y)
    [finY : FiniteRange Y] (μ : Measure Ω) [IsProbabilityMeasure μ] :
    ∃ (Ω' : Type u) (_ : MeasurableSpace Ω') (X₁ X₂ : Ω' → α) (Y' : Ω'
      → β) (ν : Measure Ω'),
  ...
  have h3 (y : β) : IdentDistrib (fun ω ↦ (X ω, y)) (⟨X,
    Y⟩) (μ[|Y ← y]) (μ[|Y ← y]) := by
  ...
  refine ⟨(α × α) × β, by infer_instance, fun ω ↦ ω.1.1, fun ω ↦ ω.1.2,
    fun ω ↦ ω.2, ν, ?_,
  ...
  · rw [condIndepFun_iff, ae_iff_of_countable]
    ...
    · let f : (α × α) × β → α × α := Prod.fst
      ...
      rw [← map_map measurable_fst measurable_fst, ← map_map measurable_snd measurable_fst,
        this]
      simp
  ...
  · rw [← sum_meas_smul_cond_fiber' hY μ]
    ...
    have h1 : IdentDistrib (fun ω ↦ (ω.1.1, ω.2)) (fun ω ↦ (ω.1.1, y)) (m y) (m y) := by
      ...
      intro _; simp
    have h2 : IdentDistrib (fun ω ↦ (ω.1.1, y)) (fun ω ↦ (X ω,
      y)) (m y) (μ[|Y ← y]) := by
...
lemma _root_.ProbabilityTheory.condIndep_copies'
    ...
    ∃ (Ω' : Type u) (_ : MeasurableSpace Ω') (X₁ X₂ : Ω' → α) (Y' : Ω'
      ...
      IsProbabilityMeasure ν ∧ Measurable X₁ ∧ Measurable X₂ ∧ Measurable Y' ∧
      CondIndepFun X₁ X₂ Y' ν ∧ IdentDistrib (⟨X₁, Y'⟩) (⟨X, Y⟩) ν μ ∧
       IdentDistrib (⟨X₂, Y'⟩) (⟨X, Y⟩) ν μ ∧ (∀ ω,
         p (X₁ ω) (Y' ω)) ∧ (∀ ω, p (X₂ ω) (Y' ω)) := by
  ...
  refine ⟨{ω // p (X₁ ω) (Y' ω) ∧ p (X₂ ω) (Y' ω)}, inferInstance, X₁ ∘ (↑),
    X₂ ∘ (↑), Y' ∘ (↑),
