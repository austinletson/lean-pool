...
/-- If $S$ is a finite set, and $a_s$, $b_s$ are non-negative for $s\in S$, then
  ...
  We require additionally that, if $b_s=0$, then $s_s=0$ as otherwise the right hand
  side should
  morally be infinite, which it can't be in the formalization using real numbers. -/
lemma sum_mul_log_div_leq {a b : ι → ℝ} (ha : ∀ i ∈ s, 0 ≤ a i) (hb : ∀ i ∈ s,
  0 ≤ b i)
...
lemma sum_mul_log_div_eq_iff_aux {a b : ι → ℝ} (ha : ∀ i ∈ s,
  0 ≤ a i) (hb : ∀ i ∈ s, 0 < b i)
    (heq : ∑ i ∈ s, a i * log (a i / b i) = (∑ i ∈ s, a i) * log ((∑ i ∈ s,
      a i) / ∑ i ∈ s, b i)) :
    ∃ r, ∀ i ∈ s, a i = r * (b i) := by
...
lemma sum_mul_log_div_eq_iff {a b : ι → ℝ} (ha : ∀ i ∈ s, 0 ≤ a i) (hb : ∀ i ∈ s,
  0 ≤ b i)
    (habs : ∀ i ∈ s, b i = 0 → a i = 0)
