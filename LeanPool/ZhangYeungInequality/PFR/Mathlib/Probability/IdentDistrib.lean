...
lemma identDistrib_id_left {X : α
  → β} (hX : AEMeasurable X μ) : IdentDistrib id X (μ.map X) μ where
  aemeasurable_fst := aemeasurable_id
  aemeasurable_snd := hX
  map_eq := by simp

lemma identDistrib_id_right {X : α
  → β} (hX : AEMeasurable X μ) : IdentDistrib X id μ (μ.map X) :=
...
-/
lemma _root_.ProbabilityTheory.IdentDistrib.fst_id
...
/--
The second projection in a product space with measure `μ.prod ν` is distributed like
`ν`.
-/
lemma _root_.ProbabilityTheory.IdentDistrib.snd_id
...
protected lemma _root_.ProbabilityTheory.IdentDistrib.cond
...
of conull
range. -/
lemma identDistrib_comp_left {i : δ → α} (hi : MeasurableEmbedding i) (hi' : ∀ᵐ a ∂μ,
  a ∈ range i)
  ...
  map_eq := by rw [← Measure.map_map hf hi.measurable, hi.map_comap,
    restrict_eq_self_of_ae_mem hi']

/-- A function is identically distributed to itself composed with a measurable embedding
of conull
range. -/
lemma identDistrib_comp_right {i : δ
  → α} (hi : MeasurableEmbedding i) (hi' : ∀ᵐ a ∂μ, a ∈ range i)
...
-- https://leanprover.zulipchat.com/#narrow/stream/217875-Is-there-code-for-X.3F
/-- Composing identically distributed functions with a measurable embedding of conull
range
gives identically distributed functions. -/
lemma _root_.ProbabilityTheory.IdentDistrib.comp_left
    {i : δ → α} (hi : MeasurableEmbedding i) (hi' : ∀ᵐ a ∂μ,
  a ∈ range i)
...
range
gives identically distributed functions. -/
lemma _root_.ProbabilityTheory.IdentDistrib.comp_right {i : δ
  → β} (hi : MeasurableEmbedding i) (hi' : ∀ᵐ a ∂ν, a ∈ range i)
    (hg : Measurable g) (hfg : IdentDistrib f g μ ν) : IdentDistrib f (g ∘ i) μ (ν.comap i) :=
  hfg.trans <| identDistrib_comp_right hi hi' hg

lemma _root_.MeasureTheory.MeasurePreserving.identDistrib {α β γ : Type*} {X : β
  → γ} {f : α → β}
...
variable {Ω Ω' α ι β β' T : Type*} {mΩ : MeasurableSpace Ω} {mΩ' : MeasurableSpace Ω'}
  {mβ : MeasurableSpace β} {μ : Measure Ω} {ν : Measure Ω'} {f g : Ω
    → β} {f' g' : Ω' → β}

variable [IsFiniteMeasure μ] [IsFiniteMeasure ν] in
theorem _root_.ProbabilityTheory.IdentDistrib.prodMk
...
lemma _root_.ProbabilityTheory.AEMeasurable.piMk
...
@[to_additive]
theorem _root_.ProbabilityTheory.IdentDistrib.mul
    (hff' : IdentDistrib f f' μ ν) (hgg' : IdentDistrib g g' μ ν)
    (h : IndepFun f g μ) (h' : IndepFun f' g' ν) :
...
/-- To show identical distribution of two random variables on a mixture of probability
measures,
...
lemma identDistrib_of_sum {X : Ω → α} {Y : Ω' → α} {μ : T → Measure Ω}
    {μ' : T → Measure Ω'} {w : T
      → ENNReal} (s : Finset T) (hX : Measurable X) (hY : Measurable Y)
...
(in the first factor). -/
lemma identDistrib_comp_fst {X : Ω
  → α} (hX : Measurable X) (μ : Measure Ω) (μ' : Measure Ω')
...
lemma identDistrib_comp_snd {X : Ω
  → α} (hX : Measurable X) (μ : Measure Ω) (μ' : Measure Ω')
  [SigmaFinite μ] [IsProbabilityMeasure μ'] :
    IdentDistrib (X ∘ Prod.snd) X (μ'.prod μ) μ where
...
lemma independent_copies' {I : Type u} [Finite I] {α : I → Type u'}
    ...
    [mΩ : ∀ i : I, MeasurableSpace (Ω i)] (X : ∀ i : I, Ω i → α i) (hX : ∀ i : I,
      Measurable (X i))
  ...
  cases nonempty_fintype I
  refine ⟨Π i, Ω i, inferInstance, .pi μ, fun i ↦ X i ∘ eval i, inferInstance, ?_,
    fun i ↦ ⟨?_, ?_⟩⟩
  · rw [iIndepFun_iff]
    intro t s hs
...
lemma independent_copies3_nondep {α : Type u}
    ...
    ∃ (A : Type (max u_1 u_2 u_3)) (_ : MeasurableSpace A) (μA :
      Measure A) (X₁' X₂' X₃' : A → α),
...
lemma independent_copies4_nondep {α : Type u}
    ...
    (hX₁ : Measurable X₁) (hX₂ : Measurable X₂) (hX₃ : Measurable X₃) (hX₄ :
      Measurable X₄)
