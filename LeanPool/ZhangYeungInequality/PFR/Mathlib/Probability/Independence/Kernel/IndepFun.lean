...
variable {β γ : ι → Type*} {m : ∀ i, MeasurableSpace (β i)} {mγ : ∀ i,
  MeasurableSpace (γ i)}
  {f : ∀ i, Ω → β i}
...
lemma _root_.ProbabilityTheory.Kernel.iIndepFun.finsets {J : Type*} [Finite J]
  ...
  let πβ (j : J) :=
    Set.pi Set.univ '' Set.pi Set.univ fun (i : S j) => { s | MeasurableSet[m i] s }
  ...
  suffices ∀ᵐ a ∂μ, κ a (⋂ j : s, ⋂ (i : S j), E' j i) = ∏ j : s,
    κ a (⋂ (i : S j), E' j i) by
    convert this with x
  ...
  set h_disjoint' :
      (↑s:Set J).PairwiseDisjoint S :=
        Set.PairwiseDisjoint.subset h_disjoint fun ⦃a⦄ _ ↦ trivial
  ...
  have Einter_mes :
      ∀ᵐ a ∂μ, κ a (⋂ j : s, ⋂ i : S j, E' j i) = ∏ j : s, ∏ i : S j,
        κ a (E' j i) := by
    rw [Einter_eq]
...
lemma _root_.ProbabilityTheory.Kernel.iIndepFun.finsets_comp {J : Type*} [Finite J]
    (S : J → Finset ι) (h_disjoint : Set.PairwiseDisjoint Set.univ S)
    (hf_Indep : iIndepFun f κ μ) (hf_meas : ∀ i, Measurable (f i))
