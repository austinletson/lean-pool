...
We can write `κ : Kernel S (T × U)` as a composition-product `(fst κ) ⊗ₖ (condKernel κ)`
where
...
lemma condKernel_map_prodMk_left {V : Type*} [Nonempty V] [MeasurableSpace V]
    ...
    condKernel (map κ (fun p ↦ (p.1, f p)))
      =ᵐ[μ ⊗ₘ fst κ] snd ((condKernel κ) ⊗ₖ (deterministic (fun x :
        (T × S) × U ↦ f (x.1.2, x.2))
          .of_discrete)) := by
  rw [Filter.EventuallyEq, ae_iff_of_countable]
  intro x hx
...
lemma _root_.ProbabilityTheory.condDistrib_apply'
    [Nonempty S] (hX : Measurable X) (hY : Measurable Y) (μ : Measure Ω)
    [IsFiniteMeasure μ] (x : T) (hYx : μ (Y ⁻¹' {x}) ≠ 0) {s : Set S} (hs :
      MeasurableSet s) :
...
lemma _root_.ProbabilityTheory.condDistrib_ae_eq
    [Nonempty S] (hX : Measurable X) (hY : Measurable Y) (μ : Measure Ω)
    [IsFiniteMeasure μ] :
...
lemma _root_.ProbabilityTheory.condDistrib_fst_of_ne_zero [Nonempty S]
    (hX : Measurable X) (hY : Measurable Y) (hZ : Measurable Z) (μ :
      Measure Ω) [IsFiniteMeasure μ]
    (u : U) (hu : μ (Z ⁻¹' {u}) ≠ 0) :
...
lemma _root_.ProbabilityTheory.condDistrib_fst_ae_eq
    [Nonempty S] (hX : Measurable X) (hY : Measurable Y) (hZ : Measurable Z)
...
lemma _root_.ProbabilityTheory.condDistrib_snd_of_ne_zero [Nonempty S]
...
lemma _root_.ProbabilityTheory.condDistrib_snd_ae_eq
    [Nonempty S] (hX : Measurable X) (hY : Measurable Y) (hZ : Measurable Z)
    (μ : Measure Ω) [IsFiniteMeasure μ] :
...
lemma _root_.ProbabilityTheory.condKernel_condDistrib_ae_eq [Nonempty S]
...
lemma _root_.ProbabilityTheory.swap_condDistrib_ae_eq
    (hX : Measurable X) (hY : Measurable Y) (hZ : Measurable Z)
    (μ : Measure Ω) [IsFiniteMeasure μ] :
...
lemma _root_.ProbabilityTheory.condDistrib_const_unit (hX : Measurable X) (hY : Measurable Y)
    (μ : Measure Ω) [IsFiniteMeasure μ] :
...
lemma _root_.ProbabilityTheory.condDistrib_unit_right [Nonempty S]
...
lemma _root_.ProbabilityTheory.map_compProd_condDistrib
    [Nonempty S] (hX : Measurable X) (hZ : Measurable Z)
  ...
  have : ∑' x : U, μ.map Z {x} * condDistrib X Z μ x (Prod.mk x ⁻¹' A)
    ...
    congr
  rw [this]
  have : (fun a ↦ (Z a, X a)) ⁻¹' A = ⋃ x, Z ⁻¹' {x} ∩ ((fun a ↦ (x,
    X a)) ⁻¹' A) := by
    ext ω
    simp
...
lemma _root_.ProbabilityTheory.condDistrib_eq_prod_of_indepFun [Nonempty S]
  ...
  have hxW : μ (W ⁻¹' {x.2}) ≠ 0 :=
    fun h0 ↦ hx (measure_mono_null Set.inter_subset_right h0)
  ...
  congr 1
  have h_Union : (fun ω ↦ (X ω, Y ω)) ⁻¹' s = ⋃ a,
    ((X ⁻¹' {a}) ∩ (Y ⁻¹' (Prod.mk a ⁻¹' s))) := by
    ext1; simp
...
lemma _root_.MeasureTheory.Measure.compProd_apply_singleton
  ...
  have : ∀ a, κ a (Prod.mk a ⁻¹' {(t,
    s)}) = ({t} : Set T).indicator (fun _ ↦ κ t {s}) a := by
    intro a
    by_cases ha : a = t
...
/-- Build an everywhere finite-support representative from an almost-everywhere one. -/
noncomputable
def _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.mk
    {μ} {κ : Kernel T S} (hκ : AEFiniteKernelSupport κ μ) :
    Kernel T S := by
  classical
...
@[simp] lemma _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.mk_zero
    {h : AEFiniteKernelSupport (0 : Kernel T S) μ} : h.mk = 0 := by
  rcases isEmpty_or_nonempty S with hS | hS
  · simp [AEFiniteKernelSupport.mk]
  ext x
  by_cases hx : x ∈ {t | ∃ A : Finset S, (0 : Kernel T S) t Aᶜ = 0} <;>
    simp [AEFiniteKernelSupport.mk, Kernel.piecewise, hS]

@[simp] lemma _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.mk_eq_zero_of_isEmpty
...
lemma _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.finiteKernelSupport_mk
    [MeasurableSingletonClass S] {κ : Kernel T S}
  ...
  intro t
  classical
  rw [AEFiniteKernelSupport.mk_eq, Kernel.piecewise_apply]
  split_ifs with ht
  · exact ht
  · refine ⟨{hS.some}, ?_⟩
    simp

lemma _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.ae_eq_mk
...
instance _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.isZeroOrMarkovKernel_mk
...
instance _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.isSFiniteKernel_mk
  ...
  rcases isEmpty_or_nonempty S with hS | hS
  · simp only [AEFiniteKernelSupport.mk_eq_zero_of_isEmpty]; infer_instance
  rw [AEFiniteKernelSupport.mk_eq]
  infer_instance

lemma aefiniteKernelSupport_iff [MeasurableSingletonClass S] {κ : Kernel T S} {μ : Measure T} :
    AEFiniteKernelSupport κ μ ↔ ∃ κ' : Kernel T S,
      FiniteKernelSupport κ' ∧ κ' =ᵐ[μ] κ := by
  refine ⟨fun h ↦ ⟨h.mk, h.finiteKernelSupport_mk, h.ae_eq_mk.symm⟩, fun ⟨κ', hκ',
    h_eq⟩ ↦ ?_⟩
...
lemma _root_.ProbabilityTheory.Kernel.local_support_of_finiteKernelSupport
...
lemma _root_.ProbabilityTheory.Kernel.finiteKernelSupport_of_finite_range
    [Finite S] (κ : Kernel T S) : FiniteKernelSupport κ :=
  by
...
lemma _root_.ProbabilityTheory.Kernel.FiniteKernelSupport.deterministic [Countable S] [Countable T]
...
lemma _root_.ProbabilityTheory.Kernel.FiniteKernelSupport.map [MeasurableSingletonClass U]
...
lemma _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.map
    [MeasurableSingletonClass U] {κ : Kernel T S} {μ : Measure T}
...
lemma _root_.ProbabilityTheory.Kernel.FiniteKernelSupport.comap
    {κ : Kernel T S} (hκ : FiniteKernelSupport κ)
...
lemma _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.comap_equiv
...
lemma _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.fst
    [MeasurableSingletonClass S] {κ : Kernel T (S × U)} {μ :
  Measure T}
    (hκ : AEFiniteKernelSupport κ μ) :
...
lemma _root_.ProbabilityTheory.Kernel.FiniteKernelSupport.snd [MeasurableSingletonClass U]
    {κ : Kernel T (S × U)} (hκ : FiniteKernelSupport κ) :
    FiniteKernelSupport (snd κ) := by
  rw [snd_eq]
  apply hκ.map

lemma _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.snd
    [MeasurableSingletonClass U] {κ : Kernel T (S × U)} {μ :
  Measure T}
    (hκ : AEFiniteKernelSupport κ μ) :
...
lemma _root_.ProbabilityTheory.Kernel.aefiniteKernelSupport_of_cond
    {κ : Kernel T (S × U)} [hU : Nonempty U]
    [MeasurableSingletonClass S] [MeasurableSingletonClass T]
...
lemma _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.swapRight
...
lemma _root_.ProbabilityTheory.Kernel.FiniteKernelSupport.prod {κ : Kernel T S} {η : Kernel T U}
    [MeasurableSingletonClass S] [MeasurableSingletonClass U]
...
lemma _root_.ProbabilityTheory.Kernel.FiniteKernelSupport.compProd
    [MeasurableSingletonClass S] [MeasurableSingletonClass U]
...
lemma _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.compProd
...
lemma _root_.ProbabilityTheory.Kernel.FiniteKernelSupport.prodMkRight
    {κ : Kernel T S} (hκ : FiniteKernelSupport κ) :
    FiniteKernelSupport (prodMkRight U κ) :=
  hκ.comap _

protected lemma _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.prodMkRight
  ...
  change ∀ᵐ (x : T × U) ∂(μ.prod ν),
    x ∈ {y | (prodMkRight U hκ.mk) y = (prodMkRight U κ) y}
...
protected lemma _root_.ProbabilityTheory.Kernel.AEFiniteKernelSupport.prodMkLeft
    [MeasurableSingletonClass S]
    [Countable T] [MeasurableSingletonClass T] [MeasurableSingletonClass U] [Countable U] {μ}
    {κ : Kernel T S} (hκ : AEFiniteKernelSupport κ μ)
  ...
  rw [Filter.EventuallyEq]
  change ∀ᵐ (x : U × T) ∂(ν.prod μ),
    x ∈ {y | prodMkLeft U hκ.mk y = prodMkLeft U κ y}
  rw [Measure.ae_prod_mem_iff_ae_ae_mem]
