...
class Moves (G : Type v) where
  ...
  /-- The option relation generated by `moves` is well-founded. -/
...
class Form (G : Type (v + 1)) extends Moves G, OfSets G fun _ ↦ True, InvolutiveNeg G,
  AddCommSemigroup G where
  ...
  moves_ofSets' (p : Player) (st : Player → Set G) [Small.{v} (st .left)] [Small.{v} (st .right)] :
      moves p !{st}= st p
  ...
  ofSets_isEndLike_iff' (p : Player) (s t : Set G) [Small s] [Small t] :
      IsEndLike p !{s | t} ↔ moves p !{s | t} = ∅
  ofSets_add_ofSets'' (s₁ t₁ s₂ t₂ : Set G) [Small s₁] [Small t₁] [Small s₂] [Small t₂] :
      !{s₁ | t₁} + !{s₂ | t₂} =
        !{(· + !{s₂ | t₂}) '' s₁ ∪ (!{s₁ | t₁} + ·) '' s₂ |
          (· + !{s₂ | t₂}) '' t₁ ∪ (!{s₁ | t₁} + ·) '' t₂}
...
instance :
    IsWellFounded _ (@Subposition G _) := inferInstanceAs (IsWellFounded _ (Relation.TransGen _))
...
macro (name := formWf) "form_wf" : tactic =>
...
theorem isEnd_def {G : Type (u + 1)} [Moves G] (p : Player) (g : G) :
    IsEnd p g = (moves p g = ∅) := by rfl

theorem not_isEnd_def {G : Type (u + 1)} [Moves G] (p : Player) (g : G) :
    ¬(IsEnd p g) ↔ (moves p g ≠ ∅) := by
  simp [isEnd_def]
...
@[simp]
protected theorem IsEndLike.neg_iff_neg {g : G} {p : Player} :
    IsEndLike p (-g) ↔ IsEndLike (-p) g :=
...
@[simp 900]
theorem moves_add (p : Player) (x y : G) :
    moves p (x + y) = (· + y) '' moves p x ∪ (x + ·) '' moves p y :=
  moves_add' p x y

instance instSmallElemMoves (p : Player) (x : G) : Small.{u} (moves p x) := smallElemMoves' p x

@[simp]
theorem moves_ofSets (p : Player) (st : Player → Set G)
    [Small.{u} (st .left)] [Small.{u} (st .right)] :
    moves p !{st}= st p := moves_ofSets' p st
...
@[simp 1100]
theorem leftMoves_ofNat (n : ℕ) [n.AtLeastTwo] : moves .left (ofNat(n) : G) = {((n - 1 : ℕ) :
    G)} := by
...
theorem mem_rightMoves_natSub_add {a b : ℕ} {y g' : G} :
  ...
  constructor <;>
    simp only [sub_eq_add_neg, moves_add, rightMoves_natCast, Set.image_empty, moves_neg,
      Player.neg_right, Set.empty_union, Set.mem_union, Set.mem_image, Set.mem_neg,
      Set.exists_neg_mem, exists_exists_and_eq_and]
  · rintro (⟨x, hx, rfl⟩ | ⟨x, hx, rfl⟩)
    · cases b <;> simp_all [leftMoves_natCast_succ]
    · exact Or.inr ⟨x, hx, rfl⟩
  · rintro (⟨b', rfl, rfl⟩ | ⟨yr, hyr, rfl⟩)
    · simp
