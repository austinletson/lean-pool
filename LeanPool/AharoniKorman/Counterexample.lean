...
lemma line_mapsTo {x y : Hollom} (hxy : (ofHollom x).2.2 = (ofHollom y).2.2) :
  ...
  intro z hz
  induction z with | h p q r =>
  simp only [Set.mem_Icc, line_toHollom] at hz ⊢
  obtain rfl := (le_of_toHollom_le_toHollom hz.1).antisymm
    (le_of_toHollom_le_toHollom hz.2)
  simp only [toHollom_le_toHollom_iff_fixed_right] at hz
  omega
...
theorem exists_finite_intersection (hC : IsChain (· ≤ ·) C) :
  ...
  simp only [Set.mem_sdiff, Set.mem_inter_iff, toHollom_mem_level_iff, and_true, Set.mem_setOf_eq,
    ofHollom_toHollom, not_le, D] at hxy
...
lemma le_apply_of_le (hC : IsChain (· ≤ ·) C) (hy : y ∈ C) (hx : y ≤ x) : y ≤ f x :=
  isChain_le_of_not_lt hC (f.mem x) hy fun hxy ↦ f.not_apply_lt (hxy.trans_le hx)

lemma apply_le_of_le (hC : IsChain (· ≤ ·) C) (hy : y ∈ C) (hx : x ≤ y) : f x ≤ y :=
  isChain_le_of_not_lt hC hy (f.mem x) fun hxy ↦ f.not_lt_apply (hx.trans_lt hxy)

lemma lt_apply_of_lt (hC : IsChain (· ≤ ·) C) (hy : y ∈ C) (hx : y < x) : y < f x :=
  isChain_lt_of_not_le hC (f.mem x) hy fun hxy ↦ f.not_apply_lt (hxy.trans_lt hx)

lemma apply_lt_of_lt (hC : IsChain (· ≤ ·) C) (hy : y ∈ C) (hx : x < y) : f x < y :=
  isChain_lt_of_not_le hC hy (f.mem x) fun hxy ↦ f.not_lt_apply (hx.trans_le hxy)
...
lemma image_chainBetween_isChain {a b c d n : ℕ} :
  ...
  rw [Finset.coe_image]
  exact chainBetween_isChain.image (embed n)
...
lemma apply_eq_of_line_eq_step (f : SpinalMap C) {n xl yl xh yh : ℕ}
  ...
  have f_inj : Set.InjOn f B := by
    ...
    simp only [B]
    simpa [Finset.coe_image, Finset.coe_union] using
      (show IsChain (· ≤ ·)
        (chainBetween xl yl x y ∪ chainBetween (x + 1) (y + 1) xh yh : Set (ℕ × ℕ)) from
        by
          rw [isChain_union]
  ...
  replace hleft : f h(x + 1, y, n) ∈ I \ B.image f := by
    simpa [Finset.mem_sdiff, hleft, I] using this.1
  replace hright : f h(x, y + 1, n) ∈ I \ B.image f := by
    simpa [Finset.mem_sdiff, hright, I] using this.2
...
lemma x0y0_min (z : ℕ × ℕ) (hC : IsChain (· ≤ ·) C) (h : embed (n + 1) z ∈ C) :
  ...
  refine isChain_le_of_not_lt hC h (x0y0_mem this) ?_
  ...
  case inr h =>
    ...
    · obtain ⟨j, hjCn, hj⟩ := rbias (a + b - x0 n C)
      ...
      have hj' : h(a, b, n + 1) ≤ j := by
        ...
        apply isChain_le_of_not_lt hC hjCn.1 hp.1
        intro h
...
theorem not_S_mapsTo_previous (hC : IsChain (· ≤ ·) C)
  ...
  have F_subs : (F : Set Hollom) ⊆ S n C \ (C ∩ level n) := calc
    (F : Set Hollom) = embed n '' chainBetween a a (3 * a) a := Finset.coe_image
