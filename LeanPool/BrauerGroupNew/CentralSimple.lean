...
lemma _root_.IsCentralSimple.TensorProduct.sum_tmul_basis_right_eq_zero'
...
lemma _root_.IsCentralSimple.TensorProduct.sum_tmul_basis_left_eq_zero'
    (B : Type*) [Ring B] [Algebra K B]
    (C : Type*) [Ring C] [Algebra K C]
  ...
  simpa using h
lemma _root_.IsCentralSimple.TensorProduct.left_tensor_base_sup_base_tensor_right
...
lemma _root_.IsCentralSimple.TensorProduct.submodule_tensor_inf_tensor_submodule
...
/-- Linear equivalence identifying the tensor product of centers with the center of a tensor
product. -/
...
instance _root_.IsCentralSimple.TensorProduct.isCentral
...
instance _root_.IsCentralSimple.TensorProduct.nontrivial
...
structure is_obtainable_by_sum_tmul
  ...
  rep : ∃ (s : Finset ιA) (_ : s.card = n) (f : ιA → B),
    x = ∑ i ∈ s, 𝒜 i ⊗ₜ[K] f i
variable {K} in
lemma _root_.IsCentralSimple.is_obtainable_by_sum_tmul.exists_minimal_element
    {A B : Type v} [Ring A] [Algebra K A] [Ring B] [Algebra K B]
...
lemma _root_.IsCentralSimple.TensorProduct.map_comap_eq_of_isSimple_isCentralSimple
    {A B : Type v} [Ring A] [Algebra K A] [Ring B] [Algebra K B]
    [isSimple_A : IsSimpleOrder <| TwoSidedIdeal A]
    [isCentral_B : Algebra.IsCentral K B]
    [isSimple_B : IsSimpleRing B]
    ...
    I = TwoSidedIdeal.span
      (Set.image (Algebra.TensorProduct.includeLeft : A →ₐ[K] A ⊗[K] B) <|
        I.comap (Algebra.TensorProduct.includeLeft : A →ₐ[K] A ⊗[K] B)) := by
  refine le_antisymm (TensorProduct.map_comap_le_span_of_isSimple_isCentralSimple (K := K) I) ?_
  rw [TwoSidedIdeal.span_le]
  rintro _ ⟨x, hx, rfl⟩
  rw [SetLike.mem_coe, TwoSidedIdeal.mem_comap] at hx
  exact hx
instance _root_.IsCentralSimple.TensorProduct.simple
    (A B : Type v) [Ring A] [Algebra K A] [Ring B] [Algebra K B]
