...
* `TriangularDecomposition.VermaHW.universalMap`: The universal property of the Verma module:
  Given another module `M` over `𝓤 𝕜 𝓰` and a "target" vector `hwv ∈ M` satisfying
  `E • hwv = 0` for all `E` in the positive part of the triangular decomposition and
  `H • hwv  = η(H) • hwv` for all `H` in the Cartan subalgebra, one obtains an `A`-module map from
  the Verma module to `M` uniquely specified by the requirement `hwVec η ↦ hwv`.
...
* `TriangularDecomposition.VermaHW.hwVec_cyclic`: The Verma module is generated by its highest
  weight vector.
...
lemma _root_.VirasoroProject.TriangularDecomposition.VermaHW.smul_eq_algebraHom_smul
    {η : weight tri} (r : 𝕜) (v : VermaHW η) :
...
lemma _root_.VirasoroProject.TriangularDecomposition.VermaHW.universalMap_hwVec (η : weight tri)
    (M : Type*) [AddCommGroup M] [Module (𝓤 𝕜 𝓰) M] {hwv : M}
    (hwv_cartan : ∀ {H} (hH : H ∈ tri.cartan),
      ιUEA 𝕜 H • hwv = (algebraMap 𝕜 (𝓤 𝕜 𝓰) (η ⟨H, hH⟩) • hwv))
    (hwv_upper : ∀ {E} (_ : E ∈ tri.upper), ιUEA 𝕜 E • hwv = 0) :
    VermaHW.universalMap η M hwv_cartan hwv_upper (VermaHW.hwVec η) = hwv :=
...
lemma _root_.VirasoroProject.HasCentralValue.central_smul
    {C : 𝓰} {c : 𝕜} [HasCentralValue 𝕜 𝓰 M C c] (v : M) :
    ιUEA 𝕜 C • v = c • v :=
  HasCentralValue.central_smul' v
