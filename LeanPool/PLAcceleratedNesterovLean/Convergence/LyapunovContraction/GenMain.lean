...
private abbrev lyapunovContractionGenProof
    {d : ℕ} (_hd : 0 < d)
  ...
  case contraction =>
    ...
    -- Ln ≥ 0
    have hLn_nn : 0 ≤ Ln := by
      exact lyapunovOfState_nonneg P μ' π f η sn hμ' hη_pos hμη_lt1 hbdd
...
theorem lyapunov_contraction_gen
    ...
    ∃ (Ω Ω_plus : Set (E d)) (R : ℝ),
      ...
      ∀ (s₀ : NesterovState d) (n : ℕ),
        ...
        -- Next iterates in Ω⁺
        s'.x ∈ Ω_plus ∧ s'.lookahead η ∈ Ω_plus := by
  exact lyapunovContractionGenProof (d := d) (_hd := _hd) (f := f) (L := L) (hL := hL) (μ' := μ')
