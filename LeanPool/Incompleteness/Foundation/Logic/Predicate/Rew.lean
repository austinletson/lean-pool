...
lemma q_substs (w : Fin n → Semiterm L ξ n') :
    (substs w).q = substs (#0 :> bShift ∘ w) := by
      ext x; { cases x using Fin.cases <;> simp }; { simp }
...
/-- Imported declaration from the Incompleteness formalization. -/
class Rewriting (L : outParam Language) (ξ : outParam Type*) (F : ℕ →
  Type*) (ζ : Type*) (G : outParam (ℕ → Type*))
    [LCWQ F] [LCWQ G] where
  /-- Imported declaration from the Incompleteness formalization. -/
...
@[simp] lemma smul_ball (ω : Rew L ξ n₁ ζ n₂) (φ ψ : F (n₁ + 1)) :
    ω ▹ (∀[φ] ψ) = ∀[ω.q ▹ φ] (ω.q ▹ ψ) := by
  simp [ball]

@[simp] lemma smul_bex (ω : Rew L ξ n₁ ζ n₂) (φ ψ : F (n₁ + 1)) :
    ω ▹ (∃[φ] ψ) = ∃[ω.q ▹ φ] (ω.q ▹ ψ) := by
  simp [bex]

/-- Imported declaration from the Incompleteness formalization. -/
abbrev substitute [Rewriting L ξ F ξ F] (φ : F n₁) (w : Fin n₁ → Semiterm L ξ n₂) :
    F n₂ :=
  Rew.substs w ▹ φ
...
/-- Imported declaration from the Incompleteness formalization. -/
@[coe] abbrev embedding {ο ξ} [IsEmpty ο] {O F : ℕ →
    Type*} [LCWQ O] [LCWQ F] [Rewriting L ο O ξ F] (φ : O n) :
    F n :=
  @Rew.emb L ο _ ξ n ▹ φ
...
class TransitiveRewriting (L : outParam Language)
    (ξ₁ : outParam Type*) (F₁ : ℕ → Type*) (ξ₂ : Type*) (F₂ : outParam (ℕ →
      Type*)) (ξ₃ : Type*) (F₃ : outParam (ℕ → Type*))
    [LCWQ F₁] [LCWQ F₂] [LCWQ F₃]
    [Rewriting L ξ₁ F₁ ξ₂ F₂] [Rewriting L ξ₂ F₂ ξ₃ F₃] [Rewriting L ξ₁ F₁ ξ₃ F₃] where
  comp_app (ω₁₂ : Rew L ξ₁ n₁ ξ₂ n₂) (ω₂₃ : Rew L ξ₂ n₂ ξ₃ n₃) (φ : F₁ n₁) : (ω₂₃.comp ω₁₂) ▹ φ =
    ω₂₃ ▹ ω₁₂ ▹ φ

/-- Imported declaration from the Incompleteness formalization. -/
class InjMapRewriting (L : outParam Language) (ξ : outParam Type*) (F : ℕ →
  Type*) (ζ : Type*) (G : outParam (ℕ → Type*))
  ...
  smul_map_injective {b : Fin n₁ → Fin n₂} {f : ξ → ζ} :
    (hb : Function.Injective b) → (hf : Function.Injective f) →
        Function.Injective fun φ : F n₁ ↦ Rew.map (L :=
      L) b f ▹ φ
...
@[simp] lemma substitute_empty (φ : S 0) (v : Fin 0 → Semiterm L ℕ 0) :
    (φ <~ v) = φ := by
  simp [substitute]
...
@[simp] lemma cast_substs_eq (t : SyntacticTerm L) (φ : S 0) :
  ...
  suffices (Rewriting.substitute (ξ :=
    ℕ) φ ![])/[t] = Rew.id ▹ φ by rwa [ReflectiveRewriting.id_app] at this
...
lemma allClosure_fixitr (φ : S 0) :
    ∀* Rew.fixitr 0 (m + 1) ▹ φ = ∀' Rew.fix ▹ (∀* Rew.fixitr 0 m ▹ φ) := by
...
@[simp] lemma emb_univClosure [Rewriting L ο O ξ F] {σ : O n} :
    (embedding (ξ := ξ) (∀* σ)) = ∀* (embedding (ξ := ξ) σ) := by
      induction n <;> simp [*, univClosure_succ]

/-- `coe_substs_eq_substs_coe` -/
lemma embedding_substitute_eq_substitute_embedding
    ...
    (embedding (ξ := ξ) (φ <~ v)) =
        Rewriting.substitute (ξ := ξ) (embedding (ξ := ξ) φ) (fun i ↦ Rew.emb (v i)) := by
...
lemma embedding_substs_eq_substs_coe₁
  ...
  simpa [Matrix.constant_eq_singleton] using embedding_substitute_eq_substitute_embedding φ ![t]
