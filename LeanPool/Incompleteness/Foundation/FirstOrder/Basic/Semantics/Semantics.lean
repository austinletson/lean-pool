...
lemma eval_rew (ω : Rew L ξ₁ n₁ ξ₂ n₂) (φ : Semiformula L ξ₁ n₁) :
  ...
  case hall =>
    ...
    exact iff_of_eq <| forall_congr fun x ↦ by
      congr; funext i; cases i using Fin.cases <;> simp
  case hex =>
    simp only [Function.comp_def, Rew.q_fvar, Semiterm.val_bShift]
    exact exists_congr fun x ↦ iff_of_eq <| by
      congr; funext i; cases i using Fin.cases <;> simp
...
lemma eval_rew_q {ε₂ : ξ₂ → M} (ω : Rew L ξ₁ n₁ ξ₂ n₂) (φ : Semiformula L ξ₁ (n₁ + 1)) :
    ...
    Eval s
      ...
      (Semiterm.val s e₂ ε₂ ∘ ω ∘ Semiterm.fvar) φ := by
  simp only [eval_rew, Function.comp_def, Nat.succ_eq_add_one, Rew.q_fvar,
    Semiterm.val_bShift]
  apply iff_of_eq; congr 2
  · funext x
    cases x using Fin.cases <;> simp

lemma eval_map (b : Fin n₁ → Fin n₂) (f : ξ₁ → ξ₂) (e : Fin n₂ → M) (ε : ξ₂ → M) (φ :
    Semiformula L ξ₁ n₁) :
...
lemma eval_toEmpty [DecidableEq ξ] {φ : Semiformula L ξ n} (hp : φ.freeVariables = ∅) :
    Eval s e f φ ↔ Evalb s e (φ.toEmpty hp) := by
  induction φ using Semiformula.rec'
  case hrel k R v =>
...
lemma eval_ofEquiv_iff : ∀ {n} {e : Fin n → N} {ε : ξ → N} {φ : Semiformula L ξ n},
  ...
  | _, e, ε, φ ⋎ ψ     => by simp [eval_ofEquiv_iff (φ := φ), eval_ofEquiv_iff (φ := ψ)]
  | _, e, ε, ∀' φ      => by
    simp only [eval_all, Nat.succ_eq_add_one]; exact
  ...
  | _, e, ε, ∃' φ      => by
    simp only [eval_ex, Nat.succ_eq_add_one]; exact
...
/-- Imported declaration from the Incompleteness formalization. -/
lemma modelsTheory_iff_modelsTheory_s : M ⊧ₘ* T ↔ s.toStruc ⊧* T := by rfl
...
lemma _root_.LO.FirstOrder.Theory.models [M ⊧ₘ* T] {φ} (hp : φ ∈ T) : M ⊧ₘ φ :=
...
lemma satisfiable_iff :
    Semantics.Satisfiable (Struc.{v, u} L) T ↔ ∃ (M :
        Type v) (_ : Nonempty M) (_ : Structure L M), M ⊧ₘ* T :=
  ⟨by
    rintro ⟨s, hs⟩
...
lemma unsatisfiable_iff :
    ¬Semantics.Satisfiable (Struc.{v, u} L) T ↔
        ∀ (M : Type v) (_ : Nonempty M) (_ : Structure L M), ¬M ⊧ₘ* T := by
...
lemma _root_.LO.FirstOrder.ModelOfSat.models (h : Semantics.Satisfiable (Struc.{v, u} L) T) :
    ModelOfSat h ⊧ₘ* T := (StructureModelOfSatAux h).prop
...
lemma lMap_models_lMap
    {L₁ L₂ : Language.{u}} {Φ : L₁ →ᵥ L₂} {T : Theory L₁} {φ : SyntacticFormula L₁} (h :
    T ⊨[Struc.{v, u} L₁] φ) :
...
lemma subset_of_models : T ⊆ theory L M ↔ M ⊧ₘ* T :=
