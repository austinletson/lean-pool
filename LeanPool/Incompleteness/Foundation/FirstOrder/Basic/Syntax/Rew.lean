...
@[simp] lemma rew_nrel2 (ω : Rew L ξ₁ n₁ ξ₂ n₂) {r : L.Rel 2} {t₁ t₂ : Semiterm L ξ₁ n₁} :
    ω ▹ nrel r ![t₁, t₂] = nrel r ![ω t₁, ω t₂] := by
      rw [rew_nrel]
      congr
      funext i
      cases i using Fin.cases with
...
private lemma map_inj {b : Fin n₁ → Fin n₂} {f : ξ₁ → ξ₂}
    (hb : Function.Injective b) (hf : Function.Injective f) : Function.Injective fun φ :
      Semiformula L ξ₁ n₁ ↦ @Rew.map L ξ₁ ξ₂ n₁ n₂ b f ▹ φ
  | ⊤,        φ => by cases φ using cases' <;> simp [rew_rel, rew_nrel]
  | ⊥,        φ => by cases φ using cases' <;> simp [rew_rel, rew_nrel]
  | rel r v,  φ => by
    cases φ using cases' <;>
      simp only [rew_rel, rew_nrel, rel.injEq, reduceCtorEq, imp_self, and_imp,
        ...
        Rewriting.app_all, Rewriting.app_ex]
    case hrel =>
      rintro rfl
  ...
  | nrel r v, φ => by
    cases φ using cases' <;>
      simp only [rew_rel, rew_nrel, nrel.injEq, reduceCtorEq, imp_self, and_imp,
        LogicalConnective.HomClass.map_top, LogicalConnective.HomClass.map_bot,
        LogicalConnective.HomClass.map_and, LogicalConnective.HomClass.map_or,
        Rewriting.app_all, Rewriting.app_ex]
    case hnrel =>
  ...
  | φ ⋏ ψ,    χ => by
    cases χ using cases' <;>
      simp only [rew_rel, rew_nrel, LogicalConnective.HomClass.map_top,
        LogicalConnective.HomClass.map_bot, LogicalConnective.HomClass.map_and,
        LogicalConnective.HomClass.map_or, Rewriting.app_all, Rewriting.app_ex, reduceCtorEq,
        imp_self, and_inj, and_imp]
  ...
  | φ ⋎ ψ,    χ => by
    cases χ using cases' <;>
      simp only [rew_rel, rew_nrel, LogicalConnective.HomClass.map_top,
        LogicalConnective.HomClass.map_bot, LogicalConnective.HomClass.map_and,
        LogicalConnective.HomClass.map_or, Rewriting.app_all, Rewriting.app_ex, reduceCtorEq,
        imp_self, or_inj, and_imp]
    intro hp hq; exact ⟨map_inj hb hf hp, map_inj hb hf hq⟩
  | ∀' φ,     ψ => by
    cases ψ using cases' <;>
      simp only [Rewriting.app_all, Rew.q_map, Nat.succ_eq_add_one,
        LogicalConnective.HomClass.map_top, LogicalConnective.HomClass.map_bot, rew_rel,
        rew_nrel, LogicalConnective.HomClass.map_and, LogicalConnective.HomClass.map_or,
        Rewriting.app_ex, reduceCtorEq, imp_self, all_inj]
    intro h; exact map_inj (b := 0 :> Fin.succ ∘ b)
      (Matrix.injective_vecCons ((Fin.succ_injective _).comp hb) (fun _ ↦ (Fin.succ_ne_zero
        _).symm)) hf h
  | ∃' φ,     ψ => by
    cases ψ using cases' <;>
      simp only [Rewriting.app_ex, Rew.q_map, Nat.succ_eq_add_one,
        LogicalConnective.HomClass.map_top, LogicalConnective.HomClass.map_bot, rew_rel,
        rew_nrel, LogicalConnective.HomClass.map_and, LogicalConnective.HomClass.map_or,
        Rewriting.app_all, reduceCtorEq, imp_self, ex_inj]
    intro h; exact map_inj (b := 0 :> Fin.succ ∘ b)
      (Matrix.injective_vecCons ((Fin.succ_injective _).comp hb) (fun _ ↦ (Fin.succ_ne_zero
        _).symm)) hf h
...
instance :
    TransitiveRewriting L ξ₁ (Semiformula L ξ₁) ξ₂ (Semiformula L ξ₂) ξ₃ (Semiformula L ξ₃) where
  comp_app {n₁ n₂ n₃ ω₁₂ ω₂₃ φ} := by
    induction φ using rec' generalizing n₂ n₃ <;> simp [rew_rel, rew_nrel, Rew.comp_app,
      Rew.q_comp, *]
...
@[simp] lemma complexity_rew (ω : Rew L ξ₁ n₁ ξ₂ n₂) (φ : Semiformula L ξ₁ n₁) :
...
lemma eq_rel_iff {φ : Semiformula L ξ₁ n₁} {k} {r : L.Rel k} {v} :
  ...
  cases φ using Semiformula.rec' <;>
    simp only [rew_rel, rel.injEq, rew_nrel, LogicalConnective.HomClass.map_top,
      LogicalConnective.HomClass.map_bot, LogicalConnective.HomClass.map_and,
      LogicalConnective.HomClass.map_or, Rewriting.app_all, Rewriting.app_ex, reduceCtorEq,
...
lemma eq_nrel_iff {φ : Semiformula L ξ₁ n₁} {k} {r : L.Rel k} {v} :
  ...
  cases φ using Semiformula.rec' <;>
    simp only [rew_nrel, nrel.injEq, rew_rel, LogicalConnective.HomClass.map_top,
      LogicalConnective.HomClass.map_bot, LogicalConnective.HomClass.map_and,
      LogicalConnective.HomClass.map_or, Rewriting.app_all, Rewriting.app_ex, reduceCtorEq,
      and_false, exists_false]
  case hnrel k' r' v =>
...
@[simp] lemma eq_neg_iff {φ : Semiformula L ξ₁ n₁} {ψ₁ ψ₂ : Semiformula L ξ₂ n₂} :
    ω ▹ φ = ψ₁ ==> ψ₂ ↔ ∃ φ₁ φ₂ : Semiformula L ξ₁ n₁, ω ▹ φ₁ = ψ₁ ∧ ω ▹ φ₂ = ψ₂ ∧ φ =
        φ₁ ==> φ₂ := by
...
lemma coe_substitute_eq_substitute_coe (φ : Semisentence L k) (v : Fin k → Semiterm L Empty n) :
    (↑(φ <~ v) : SyntacticSemiformula L n) =
      (↑φ : SyntacticSemiformula L k)<~(fun i ↦ (↑(v i) : Semiterm L ℕ n)) :=
...
lemma coe_substs_eq_substs_coe₁ (φ : Semisentence L 1) (t : Semiterm L Empty n) :
    (↑(φ/[t]) : SyntacticSemiformula L n) =
      (↑φ : SyntacticSemiformula L 1)/[(↑t : Semiterm L ℕ n)] :=
...
def formulaRec {C : SyntacticFormula L → Sort _}
  ...
  | .nrel r v => nrel r v
  | φ ⋏ ψ     =>
    and φ ψ (formulaRec verum falsum rel nrel and or all ex φ) (formulaRec verum falsum rel nrel
      and or all ex ψ)
  | φ ⋎ ψ     =>
    or φ ψ  (formulaRec verum falsum rel nrel and or all ex φ) (formulaRec verum falsum rel nrel
      and or all ex ψ)
...
lemma «fvar?_rew» [DecidableEq ξ₁] [DecidableEq ξ₂]
  ...
  case hrel n k r v =>
    ...
    · left; exact h
    · right; exact ⟨z, by simp only [fvar?_rel]; exact ⟨i, hi⟩, hz⟩
  case hnrel n k r v =>
    have : ∃ i, (ω (v i)).FVar? x := by simpa [rew_nrel, fvar?_rel] using h
    rcases this with ⟨i, hi⟩
    rcases Semiterm.fvar?_rew hi with (h | ⟨z, hi, hz⟩)
    · left; exact h
    · right; exact ⟨z, by simp only [fvar?_nrel]; exact ⟨i, hi⟩, hz⟩
  ...
  case hall n φ ihp =>
    have :
        (Rew.bind (#0 :> fun i ↦ Rew.bShift (ω #i)) (fun z ↦ Rew.bShift (ω &z)) ▹ φ).FVar? x := by
      simp only [Rewriting.app_all, fvar?_all] at h; exact h
  ...
  case hex n φ ihp =>
    have :
        (Rew.bind (#0 :> fun i ↦ Rew.bShift (ω #i)) (fun z ↦ Rew.bShift (ω &z)) ▹ φ).FVar? x := by
      simp only [Rewriting.app_ex, fvar?_ex] at h; exact h
    rcases ihp this with (⟨z, hz⟩ | ⟨z, hz⟩)
    · cases z using Fin.cases
      case zero => simp at hz
      case succ i =>
...
lemma rew_eq_of_funEqOn [DecidableEq ξ₁] {ω₁ ω₂ : Rew L ξ₁ n₁ ξ₂ n₂} {φ : Semiformula L ξ₁ n₁}
  (hb : ∀ x, ω₁ #x = ω₂ #x) (hf : Function.funEqOn φ.FVar? (ω₁ ∘ Semiterm.fvar) (ω₂ ∘
    Semiterm.fvar)) :
    ω₁ ▹ φ = ω₂ ▹ φ := by
  induction φ using rec' generalizing n₂
  case hverum => simp
  ...
  case hand ihp ihq =>
    ...
    exact ⟨ihp hb (hf.of_subset fun x hx ↦ by simp [hx]),
      ihq hb (hf.of_subset fun x hx ↦ by simp [hx])⟩
  case hor ihp ihq =>
  ...
  case hall ih =>
    ...
    exact ih (fun x ↦ by cases x using Fin.cases <;> simp [hb])
      (fun x hx ↦ by simp only [Function.comp_apply, Rew.q_fvar]; exact congr_arg _ (hf x hx))
  case hex ih =>
...
private lemma «not_fvar?_fixitr_fvSup» (φ : SyntacticFormula L) :
...
@[simp] lemma substs_comp_fixitr_eq_map (φ : SyntacticFormula L) (f : ℕ → SyntacticTerm L) :
  ...
  unfold Rewriting.substitute; rw [← TransitiveRewriting.comp_app]
  apply rew_eq_of_funEqOn
  · simp
  · intro x hx
    simp [Rew.comp_app, Rew.fixitr_fvar, Semiformula.lt_fvSup_of_fvar? hx]

@[simp 1100] lemma substs_comp_fixitr (φ : SyntacticFormula L) :
...
def toEmpty [DecidableEq ξ] {n : ℕ} : (φ :
  ...
  | rel R v,  h =>
    rel R fun i ↦ (v i).toEmpty (by
      simp only [freeVariables_rel, Finset.biUnion_eq_empty, Finset.mem_univ, forall_const] at h
      exact h i)
  | nrel R v, h =>
    nrel R fun i ↦ (v i).toEmpty (by
      simp only [freeVariables_nrel, Finset.biUnion_eq_empty, Finset.mem_univ, forall_const] at h
      exact h i)
  | ⊤,        _ => ⊤
  | ⊥,        _ => ⊥
  | φ ⋏ ψ,    h =>
    φ.toEmpty (by simp [show φ.freeVariables = ∅ ∧ ψ.freeVariables =
      ∅ by simpa [Finset.union_eq_empty] using h]) ⋏
    ψ.toEmpty (by simp [show φ.freeVariables = ∅ ∧ ψ.freeVariables =
      ∅ by simpa [Finset.union_eq_empty] using h])
  | φ ⋎ ψ,    h =>
    φ.toEmpty (by simp [show φ.freeVariables = ∅ ∧ ψ.freeVariables =
      ∅ by simpa [Finset.union_eq_empty] using h]) ⋎
...
variable {ξ : Type*} {Φ : L₁ →ᵥ L₂}

lemma lMap_bind (b : Fin n₁ → Semiterm L₁ ξ₂ n₂) (e : ξ₁ → Semiterm L₁ ξ₂ n₂) (φ :
    Semiformula L₁ ξ₁ n₁) :
    lMap Φ (Rew.bind b e ▹ φ) =
        Rew.bind (Semiterm.lMap Φ ∘ b) (Semiterm.lMap Φ ∘ e) ▹ (lMap Φ φ) := by
  induction φ using rec' generalizing ξ₂ n₂ <;>
  simp [*, rew_rel, rew_nrel, lMap_rel, lMap_nrel, Semiterm.lMap_bind, Rew.q_bind,
    Matrix.comp_vecCons', Semiterm.lMap_bShift, Function.comp_def]
...
lemma lMap_fix (φ : SyntacticSemiformula L₁ n) :
    lMap Φ (@Rew.fix L₁ n ▹ φ) = @Rew.fix L₂ n ▹ lMap Φ φ := by
...
lemma lMap_emb {ο : Type _} [IsEmpty ο] (φ : Semiformula L₁ ο n) :
    (lMap Φ (Rewriting.embedding φ : Semiformula L₁ ξ n)) = Rewriting.embedding (lMap Φ φ) :=
      lMap_bind _ _ _
...
lemma lMap_rewriteMap (φ : Semiformula L₁ ξ₁ n) (f : ξ₁ → ξ₂) :
    lMap Φ (Rew.rewriteMap (L := L₁) (n := n) f ▹ φ) =
        (Rew.rewriteMap (L := L₂) (n := n) f) ▹ (lMap Φ φ) := by
  simp [Rew.rewriteMap, lMap_rewrite, Function.comp_def]

end «lp_section_3»

@[simp] lemma rew_open_iff {ω : Rew L ξ₁ n₁ ξ₂ n₂} {φ : Semiformula L ξ₁ n₁} :
    (ω ▹ φ).Open ↔ φ.Open := by
  induction φ using Semiformula.rec' <;> simp [rew_rel, rew_nrel, open_top, open_bot, open_rel,
    open_nrel, *]
