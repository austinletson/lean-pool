...
lemma _root_.LO.Modal.Kripke.filterEquiv.equivalence : Equivalence (filterEquiv M T) where
...
omit [T.SubformulaClosed] in
lemma _root_.LO.Modal.Kripke.FilterEqvQuotient.finite (T_finite : T.Finite) :
    Finite (FilterEqvQuotient M T) := by
  have : Finite (𝒫 T) := Set.Finite.powerset T_finite
  let f : FilterEqvQuotient M T → 𝒫 T :=
    fun (Qx : FilterEqvQuotient M T) =>
      Quotient.lift (fun x => ⟨{ φ ∈ T | x ⊧ φ }, (by simp_all)⟩) (by
      intro x y hxy;
...
class FilterOf (FM : Model) (M : Model) (T : FormulaSet ℕ) [T.SubformulaClosed] : Prop where
  ...
  def_rel₁ : ∀ {x y : M.toFrame}, x ≺ y →
      Frame.Rel' (cast def_world.symm ⟦x⟧) (cast def_world.symm ⟦y⟧) := by
    tauto;
  def_box : ∀ {Qx Qy : FM.World}, Qx ≺ Qy → Quotient.lift₂ (fun x y =>
    ∀ φ, □φ ∈ T → (x ⊧ □φ → y ⊧ φ)) (by
    intro x₁ y₁ x₂ y₂ hx hy;
    apply eq_iff_iff.mpr;
    constructor;
    · intro h φ hp sp₂; exact hy φ (FormulaSet.SubformulaClosed.mem_box hp) |>.mp <| h φ hp <|
        hx (□φ) hp |>.mpr sp₂;
    · intro h φ hp sp₁; exact hy φ (FormulaSet.SubformulaClosed.mem_box hp) |>.mpr <| h φ hp <|
        hx (□φ) hp |>.mp sp₁;
  ) (cast def_world Qx) (cast def_world Qy)
  def_valuation Qx a : (ha : (atom a) ∈ T := by trivial) →
    FM Qx a ↔ Quotient.lift (fun x => M x a) (by
      intro x y h;
      apply eq_iff_iff.mpr;
      constructor;
...
variable {M T} {FM : Model}

lemma reflexive_filterOf_of_reflexive (h_filter : FilterOf FM M T) (hRefl : Std.Refl M.toFrame) :
    Std.Refl FM.Rel := by
  constructor
  intro Qx;
  obtain ⟨x, hx⟩ := Quotient.exists_rep (cast (h_filter.def_world) Qx);
  convert h_filter.def_rel₁ <| hRefl.refl x <;> simp_all;

lemma serial_filterOf_of_serial (h_filter : FilterOf FM M T) (hSerial : Serial M.toFrame) :
    Serial FM.Rel := by
  intro Qx;
  obtain ⟨x, hx⟩ := Quotient.exists_rep (cast (h_filter.def_world) Qx);
...
/-- Imported declaration from the Incompleteness formalization. -/
abbrev standardFilterationValuation (Qx : FilterEqvQuotient M T) (a : ℕ) := (ha :
    (atom a) ∈ T) → Quotient.lift (fun x => M x a) (by
  intro x y h;
  apply eq_iff_iff.mpr;
  constructor;
  · intro hx; exact h a ha |>.mp hx;
...
/-- Imported declaration from the Incompleteness formalization. -/
abbrev coarsestFilterationFrame (M : Model) (T : FormulaSet ℕ) [T.SubformulaClosed] :
    Kripke.Frame where
  World := FilterEqvQuotient M T
  Rel Qx Qy :=
    Quotient.lift₂ (fun x y => ∀ φ, □φ ∈ T → (x ⊧ □φ → y ⊧ φ)) (by
      intro x₁ y₁ x₂ y₂ hx hy;
      apply eq_iff_iff.mpr;
      constructor;
      · intro h φ hp sp₂; exact hy φ (FormulaSet.SubformulaClosed.mem_box hp) |>.mp <| h φ hp <|
...
/-- Imported declaration from the Incompleteness formalization. -/
abbrev coarsestFilterationModel (M : Model) (T : FormulaSet ℕ) [T.SubformulaClosed] :
    Kripke.Model where
...
theorem filteration {x : M.World} {φ : Formula ℕ} (hs : φ ∈ T) :
  ...
  | himp φ ψ ihp ihq =>
    constructor;
    · rintro hxy hp;
      exact ihq (FormulaSet.SubformulaClosed.mem_imp₂ hs) |>.mp <|
          hxy (ihp (FormulaSet.SubformulaClosed.mem_imp₁ hs) |>.mpr hp);
    · rintro hxy hp;
      exact ihq (FormulaSet.SubformulaClosed.mem_imp₂ hs) |>.mpr <|
          hxy (ihp (FormulaSet.SubformulaClosed.mem_imp₁ hs) |>.mp hp);
  | _ => trivial
