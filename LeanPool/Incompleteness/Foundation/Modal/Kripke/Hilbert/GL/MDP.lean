...
lemma through_original_root {x : (MDPCounterexampleFrame F₁ F₂).World} (h :
    (MDPCounterexampleFrame F₁ F₂).root ≺ x)
  : (x = F₁.root ∨ (Sum.inr (Sum.inl F₁.root) ≺ x)) ∨ (x =
...
lemma MDP_Aux (h : (□''X) *⊢[Hilbert.GL]! □φ₁ ⋎ □φ₂) :
    (□''X) *⊢[Hilbert.GL]! □φ₁ ∨ (□''X) *⊢[Hilbert.GL]! □φ₂ := by
  ...
  have : (Hilbert.GL ⊢! ⊡c ==> φ₁) ⋎ (Hilbert.GL ⊢! ⊡c ==> φ₂) := by
    ...
    obtain ⟨M₂, hM₂⟩ :=
      Hilbert.GL.Kripke.iff_unprovable_exists_unsatisfies_FiniteTransitiveTree.mp h₂;
    replace hM₁ :=
      @Kripke.MDPCounterexampleModel.modal_equivalence_original_world₁
        (M₁ := M₁) (M₂ := M₂) M₁.root (⊡c ⋏ ∼φ₁)
        |>.mp <| Formula.Kripke.Satisfies.not_imp.mp hM₁;
    replace hM₂ :=
      @Kripke.MDPCounterexampleModel.modal_equivalence_original_world₂
        (M₁ := M₁) (M₂ := M₂) M₂.root (⊡c ⋏ ∼φ₂)
        |>.mp <| Formula.Kripke.Satisfies.not_imp.mp hM₂;
    ...
    have hc : Satisfies M.toModel M.root (□c) := by
      intro x Rrx;
      rcases Kripke.MDPCounterexampleFrame.through_original_root Rrx with ((rfl | Rrx) | (rfl |
        Rrx))
    ...
    have : ¬(Satisfies M.toModel M.root (□φ₁ ⋎ □φ₂)) := by
      ...
      push Not;
      exact ⟨hp₁, hp₂⟩;
    have : ¬(Satisfies M.toModel M.root (□c ==> (□φ₁ ⋎ □φ₂))) := Classical.not_imp.mpr ⟨hc, this⟩;
...
theorem modal_disjunctive (h : Hilbert.GL ⊢! □φ₁ ⋎ □φ₂) : Hilbert.GL ⊢! φ₁ ∨ Hilbert.GL ⊢! φ₂ := by
  have : ∅ *⊢[Hilbert.GL]! □φ₁ ∨ ∅ *⊢[Hilbert.GL]! □φ₂ := by
