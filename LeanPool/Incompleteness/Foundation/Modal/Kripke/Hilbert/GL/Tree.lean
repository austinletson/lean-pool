...
lemma valid_on_FiniteTransitiveTreeClass_of_valid_on_TransitiveIrreflexiveFrameClass (h :
    Kripke.TransitiveIrreflexiveFiniteFrameClass ⊧ φ)
...
lemma satisfies_at_root_on_FiniteTransitiveTree (h : ∀ T : FiniteTransitiveTree, T.toFrame ⊧ φ) :
    ∀ M :
    FiniteTransitiveTreeModel, Satisfies M.toModel M.root φ := by
  intro M;
...
lemma valid_on_TransitiveIrreflexiveFrameClass_of_satisfies_at_root_on_FiniteTransitiveTree
  : (∀ M : FiniteTransitiveTreeModel, Satisfies M.toModel M.root φ) →
      TransitiveIrreflexiveFiniteFrameClass ⊧ φ := by
  ...
  apply Model.TransitiveTreeUnravelling.modal_equivalence_at_root (M :=
    (M↾r).toModel) (Frame.PointGenerated.rel_transitive F_trans) ⟨r, by tauto⟩ |>.mp;
  exact H ⟨(F.FiniteTransitiveTreeUnravelling F_trans F_irrefl r),
...
theorem iff_provable_satisfies_FiniteTransitiveTree : Hilbert.GL ⊢! φ ↔ (∀ M :
    FiniteTransitiveTreeModel, Satisfies M.toModel M.root φ) := by
  constructor;
  · intro h M;
