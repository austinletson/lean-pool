        ...
        exact ⟨p.fvarEnumInv, Rew.rewriteMap p.fvarEnum ▹ p, by simp[hp],
          by  intro x
              ...
              exact Semiformula.eval_iff_of_funEqOn p (by
                intro z hz
                simp [Semiformula.fvarEnumInv_fvarEnum (Semiformula.mem_fvarList_iff_fvar?.mpr
                  hz)])⟩) zero succ
...
instance div_definable : Sg0-Function₂ ((· / ·) : V → V → V) := div_defined.to_definable _
...
lemma sqrt_exists_unique (a : V) : ∃! x, x * x ≤ a ∧ a < (x + 1) * (x + 1) := by
  have : ∃ x, x * x ≤ a ∧ a < (x + 1) * (x + 1) := by
    have : a < (a + 1) * (a + 1) → ∃ x, x * x ≤ a ∧ a < (x + 1) * (x + 1) := by
      simpa using open_leastNumber (P := fun x ↦ x * x ≤ a) ⟨“x. x * x ≤ &a”, by simp, by simp⟩
...
def _root_.LO.FirstOrder.Arith.sqrtDef : Sg0.Semisentence 2 :=
  .mkSigma “b a. b * b ≤ a ∧ a < (b + 1) * (b + 1)” (by simp[])

lemma sqrt_defined : Sg0-Function₁ (fun a : V ↦ √a) via sqrtDef := by
...
lemma pair_defined : Sg0-Function₂ (fun a b : V ↦ ⟪a, b⟫) via pairDef := by
...
instance pair_definable : Sg0-Function₂ (pair : V → V → V) := pair_defined.to_definable
...
@[simp] lemma unpair_pair (a b : V) : unpair ⟪a, b⟫ = (a, b) := by
  ...
  · have : √(b * b + a) = b :=
    sqrt_eq_of_le_of_le (by simp) (by
...
lemma pi₁_defined : Sg0-Function₁ (pi₁ : V → V) via pi₁Def := by
  intro v
...
instance pi₁_definable : Sg0-Function₁ (pi₁ : V → V) := pi₁_defined.to_definable₀
  ...
  · by_cases h₂ : a₂ < b
    ...
    · simp only [h₂, ↓reduceIte]
      calc
        ...
        _          ≤ a₂ * a₂ + a₂     :=
          add_le_add (mul_le_mul (by simpa using h₂) (by simpa using h₂) (by simp) (by simp)) (by
            simpa using h₂)
        _          ≤ a₂ * a₂ + a₂ + b := by simp
...
@[simp] lemma pair_ext_iff {a₁ a₂ b₁ b₂ : V} : ⟪a₁, b₁⟫ = ⟪a₂, b₂⟫ ↔ a₁ = a₂ ∧ b₁ = b₂ :=
  ⟨fun e ↦ ⟨by simpa using congr_arg (π₁ ·) e, by simpa using congr_arg (π₂ ·) e⟩,
...
def _root_.LO.FirstOrder.Arith.pair₄Def : Sg0.Semisentence 5 :=
  .mkSigma “p a b c d. ∃ bcd <⁺ p, ∃ cd <⁺ bcd, !pairDef p a bcd ∧ !pairDef bcd b cd ∧
    !pairDef cd c d” (by simp)

/-- Imported declaration from the Incompleteness formalization. -/
def _root_.LO.FirstOrder.Arith.pair₅Def : Sg0.Semisentence 6 :=
  .mkSigma “p a b c d e. ∃ bcde <⁺ p, ∃ cde <⁺ bcde, ∃ de <⁺ cde, !pairDef p a bcde ∧
    !pairDef bcde b cde ∧ !pairDef cde c de ∧ !pairDef de d e” (by simp)
...
lemma pair₄_defined : Sg0-Function₄ ((⟪·, ·, ·, ·⟫) : V → V → V → V → V) via pair₄Def := by
  intro v
  simp only [Fin.isValue, Fin.succ_zero_eq_one, Fin.succ_one_eq_two, Fin.succ_two_eq_three,
...
lemma pair₅_defined : Sg0.DefinedFunction (fun v :
    Fin 5 → V ↦ (⟪v 0, v 1, v 2, v 3, v 4⟫)) pair₅Def := by
  intro v
  simp only [Fin.isValue, Fin.succ_zero_eq_one, Fin.succ_one_eq_two, Fin.succ_two_eq_three,
    Fin.reduceSucc, Nat.reduceAdd, pair₅Def, Nat.succ_eq_add_one,
    HierarchySymbol.Semiformula.val_mkSigma, Semiformula.eval_bexLTSucc', Semiterm.val_bvar,
...
lemma pair₆_defined : Sg0.DefinedFunction (fun v :
    Fin 6 → V ↦ (⟪v 0, v 1, v 2, v 3, v 4, v 5⟫)) pair₆Def := by
  intro v
  simp only [Fin.isValue, Fin.succ_zero_eq_one, Fin.succ_one_eq_two, Fin.succ_two_eq_three,
    Fin.reduceSucc, Nat.reduceAdd, pair₆Def, Nat.succ_eq_add_one,
    HierarchySymbol.Semiformula.val_mkSigma, Semiformula.eval_bexLTSucc', Semiterm.val_bvar,
...
def _root_.LO.FirstOrder.Arith.unNpairDef : {n : ℕ} → (i : Fin n) → Sg0.Semisentence 2
  | 0,     i => i.elim0
  | n + 1, i =>
    Fin.cases pi₁Def (fun i ↦ .mkSigma “z v. ∃ r <⁺ v, !pi₂Def r v ∧
      !(unNpairDef i) z r” (by simp)) i

lemma unNpair_defined {n} (i : Fin n) : Sg0-Function₁ (unNpair i : V → V) via unNpairDef i := by
  induction n with
  | zero => exact i.elim0
  | succ n ih =>
    intro v
...
@[definability] instance unNpair_definable {n} (i : Fin n) (Γ) : Γ-Function₁ (unNpair i : V → V) :=
...
lemma hierarchy_polynomial_induction (Γ m) [V ⊧ₘ* Theory.indScheme ℒₒᵣ (Arith.Hierarchy Γ m)]
  ...
  case ind x IH =>
    ...
    · have : x / 2 < x := div_lt_of_pos_of_one_lt pos one_lt_two
      ...
      · simpa [←hx] using even (x / 2) (by by_contra A; simp at A; simp [show x =
        0 from by simpa [A] using hx] at pos) (IH (x / 2) this)
      · simpa [←hx] using odd (x / 2) (IH (x / 2) this)

end «lp_section_8»

@[elab_as_elim] lemma hierarchy_polynomial_induction_oRing_sigma₀ [V ⊧ₘ* 𝐈Sg0] {P : V → Prop} (hP :
    Sg0-Predicate P)
    (zero : P 0) (even : ∀ x > 0, P x → P (2 * x)) (odd : ∀ x, P x → P (2 * x + 1)) : ∀ x, P x :=
  hierarchy_polynomial_induction Sg 0 (P := P) hP zero even odd

@[elab_as_elim] lemma hierarchy_polynomial_induction_oRing_sigma₁ [V ⊧ₘ* 𝐈Sg1] {P : V → Prop} (hP :
    Sg1-Predicate P)
    (zero : P 0) (even : ∀ x > 0, P x → P (2 * x)) (odd : ∀ x, P x → P (2 * x + 1)) : ∀ x, P x :=
  hierarchy_polynomial_induction Sg 1 (P := P) hP zero even odd

@[elab_as_elim] lemma hierarchy_polynomial_induction_oRing_pi₁ [V ⊧ₘ* 𝐈Pg1] {P : V → Prop} (hP :
    Pg1-Predicate P)
    (zero : P 0) (even : ∀ x > 0, P x → P (2 * x)) (odd : ∀ x, P x → P (2 * x + 1)) : ∀ x, P x :=
  hierarchy_polynomial_induction Pg 1 (P := P) hP zero even odd
