...
def _root_.LO.FirstOrder.Arith.sppow2Def : Sg0.Semisentence 1 :=
  ...
  “ m. ¬!lenbitDef 1 m ∧ !lenbitDef 2 m ∧
    ∀ i <⁺ m, !pow2Def i → 2 < i → (!lenbitDef i m ↔
      ∃ s <⁺ i, !sqrtDef s i ∧ s * s = i ∧ !lenbitDef s m)
  ” (by simp)

lemma sppow2_defined : Sg0-Predicate (SPPow2 : V → Prop) via sppow2Def := by
  intro v
...
lemma ppow2_defined : Sg0-Predicate (PPow2 : V → Prop) via ppow2Def := by
  intro v
  simp [PPow2, ppow2Def, lenbit_defined.df.iff, pow2_defined.df.iff,
    sppow2_defined.df.iff, numeral_eq_natCast]

instance ppow2_definable : Sg0-Predicate (PPow2 : V → Prop) := ppow2_defined.to_definable
...
lemma of_sqrt (hm : SPPow2 m) {i : V} (pi : Pow2 i) (him : i ≤ m) (hsqi : (√i) ^ 2 = i) (hi :
    LenBit (√i) m) :
    LenBit i m := by
  by_cases ne1 : i = 1
...
lemma sq_le_of_lt (hm : SPPow2 m) {i j : V} (pi : Pow2 i) (pj : Pow2 j) (hi : LenBit i m) (hj :
    ...
    i < j → i ^ 2 ≤ j := by
  intro hij
  suffices ∀ i < j, Pow2 i → Pow2 j → LenBit i m → LenBit j m →
      i ^ 2 ≤ j from this i hij pi pj hi hj
        ...
        have : i ≤ √j := by
          simpa [hm.sq_sqrt_eq hi pi ine2] using
            IH (√j) (sqrt_lt_self_of_one_lt (hm.one_lt hj)) (√i) this
              (pi.sqrt (hm.sq_sqrt_eq hi pi ine2)) (pj.sqrt (hm.sq_sqrt_eq hj pj jne2)) (hm.sqrt hi
                pi ine2) (hm.sqrt hj pj jne2)
...
lemma sqrt {i : V} (ppi : PPow2 i) (ne2 : i ≠ 2) : PPow2 (√i) := by
  ...
  have : LenBit i m ↔ (√i) ^ 2 = i ∧ LenBit (√i) m :=
    sppm.lenbit_iff him.le pi (lt_of_le_of_ne (one_lt_iff_two_le.mp <|
        sppm.one_lt him) (Ne.symm ne2))
  ...
  have : SPPow2 (m % (2 * √i)) :=
    ⟨ by simpa [LenBit.mod] using sppm.not_lenbit_one,
      ...
      by  intro j hj pj lt2
          ...
          have : LenBit j m ↔ (√j) ^ 2 = j ∧ LenBit (√j) m :=
            sppm.lenbit_iff (le_trans hj (by simp)) pj lt2
          rw [LenBit.mod_pow2, this]
          · constructor
            · rintro ⟨hsqj, hlen⟩
              have psqj : Pow2 (√j) := pj.sqrt hsqj
              have hsqrt_lt : √j < 2 * √i := lt_of_le_of_lt (sqrt_le_self j) hjsi
...
protected lemma sq {i : V} (ppi : PPow2 i) : PPow2 (i ^ 2) := by
  ...
  have : m < i ^ 2 :=
    lt_of_lt_of_le hm
      (two_mul_le_sq <| one_lt_iff_two_le.mp <| lt_of_le_of_ne (pos_iff_one_le.mp <|
          ppi.pos) (Ne.symm ne1))
  ...
  case ind j IH =>
    ...
    · by_cases ei : i = 2
      ...
      · have : √i < √j := by
          by_contra A
          have : j ≤ i := by
            simpa [hi.sq_sqrt_eq ei,
              hj.sq_sqrt_eq ej] using sq_le_sq.mpr (show √j ≤ √i from by simpa using A)
          exact False.elim ((not_lt.mpr this) (by simpa using hij))
        have : i ≤ √j := by
