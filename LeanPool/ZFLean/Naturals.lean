...
theorem lt_le_iff {n m} : n ≤ m ↔ n < succ m := by
  ...
  · rintro (_ | rfl)
    ...
    · exact lt_succ
  · intro h
    change n.val ∈ insert m.val m.val at h
...
@[simp] theorem add_left_eq_self {a b : ZFNat} : a + b = b ↔ a = 0 := by
  ...
  rw [add_right_cancel]
theorem add_right_eq_self {a b : ZFNat} : a + b = a ↔ b = 0 := by
  conv => lhs; rhs; rw [← @add_zero a]
  rw [add_left_cancel]
theorem self_eq_add_right {a b : ZFNat} : a = a + b ↔ b = 0 := by
...
theorem _root_.ZFSet.ZFNat.le.dest {n m : ZFNat} : n ≤ m → ∃ k, n + k = m := by
  intro h
  induction n with
...
theorem _root_.ZFSet.ZFNat.le.intro {n m k : ZFNat} (h : n + k = m) : n ≤ m := h ▸ le_add_right n k
...
theorem add_sub_cancel (n m : ZFNat) : n + m - m = n :=
...
theorem _root_.ZFSet.Nat.is_transitive : transitive Nat := by
  intro n hn
...
theorem _root_.ZFSet.ZFNat.ofNat_inj {n m : ℕ} : (n : ZFNat) = (m : ZFNat) ↔ n = m where
  mp := by
    intro h
    induction n generalizing m with
    | zero =>
